...
### 4.4 Hallazgo estructural — Prepago no tiene un flujo que espere a que el pedimento exista

Para Crédito y Contra Entrega, existe un flujo "normal" que factura después de la inspección en almacén — es decir, después de que cualquier pedimento (de importación o exportación) ya es real. Ese flujo puede servir de respaldo si una venta resulta ser Ruta B: se bloquea la facturación por adelantado y se deja que siga el curso normal.

**Prepago no tiene ese mismo respaldo.** Tanto en Prepago normal como en Prepago con adelantada, la factura **siempre** se genera antes de comprarle al proveedor — no existe, dentro de Prepago, ningún camino que facture "después", con el pedimento de exportación ya confirmado. Si un Pedido de Prepago resulta ser Ruta B, ningún camino actual de Prepago permite esperar a que ese pedimento exista. Este hallazgo se documenta en el análisis de operación de exportación (documento aparte) para tratarse en sesión con el cliente — no está resuelto aquí.
...
**Pendiente, para sesión con el cliente (ver documento de análisis de operación de exportación):** Prepago no tiene un flujo que espere a que el pedimento de exportación exista antes de facturar (sección 4.4) — a diferencia de Crédito/Contra Entrega, que sí lo tienen en su flujo normal.
