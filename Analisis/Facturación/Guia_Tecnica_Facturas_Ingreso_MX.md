...
**⚠️ Pregunta pendiente para el cliente — sincronización de estatus con Legacy (SAP):** para Crédito/Contra Entrega adelantado, la Factura se genera en ProquifaNet 2 pero su cobranza y Complemento de Pago se generan en Legacy (SAP). Eso significa que, en algún momento, `catFacturaEstatusCobranza` (sección 7.8) de esas Facturas específicas necesita pasar de `PENDIENTE_COBRO` a `COBRADA` — pero hoy no existe ninguna definición de **quién o qué actualiza ese dato**. Se necesita confirmar con el cliente: ¿existe (o se planea) una interfaz/sincronización entre Legacy y ProquifaNet 2 para transferir este estatus, o ProquifaNet 2 simplemente no lo va a saber nunca para estos casos? Sin esto, cualquier reporte de "facturas pendientes de cobro" en ProquifaNet 2 sería incorrecto para este subconjunto de Facturas.
...
| `Serie` / `Folio` | Pendiente de definir — tema aparte a abordar (ver checklist maestro) | — |
...
**⚠️ La verificación de "¿ya existe una factura?" debe ser universal, no exclusiva de Prepago con adelantada.** Una factura también puede ya existir por haber nacido bajo la excepción del `MontoAplazable` (sección 7.2.1) — cuando esto ocurre, la factura nace en PPD con un remanente real pendiente de cobrar, exactamente igual que una de Prepago con adelantada. Si el sistema solo revisa esta condición para el caso "adelantada" y no para el caso "nació con remanente por `MontoAplazable`", el pago posterior que cubra ese remanente generaría por error una factura nueva en vez de un Complemento de Pago contra la que ya existe.

```
SI NO existe una factura previa para este pedido
Y la Proforma se cubre SOLO con Cobro(s), sin ninguna NC, cubriendo el 100%:
...
O el faltante cae dentro del MontoAplazable (sección 7.2.1):
    → La factura nace directamente en PPD, FormaPago=99
    → Se generan de inmediato el/los Complemento(s) de Pago correspondientes
      por lo que sí se cubrió (uno con FormaPago=23 por la porción de NC,
      si aplica, u otro con la forma real por el Cobro recibido)
    → Si fue por MontoAplazable, queda un Saldo Insoluto real por el faltante,
      sin Complemento de Pago que lo cubra todavía

SI YA existe una factura PPD previa — sin importar POR QUÉ ya existe
(Prepago con adelantada, O nació antes por la excepción del MontoAplazable):
...
**Validación que el sistema debe forzar:** antes de ejecutar la acción de `Validar Cobro`, el sistema debe (1) verificar si ya existe una factura para este pedido — sin importar el motivo por el que ya exista — y (2) resolver el algoritmo de reparto (sección 7.4) para saber si alguna NC participó. Estas 2 condiciones determinan cuál de las 3 rutas ejecutar, nunca una elección manual del usuario.

**Validación adicional — no volver a tramitar un Pedido ya tramitado:** un Pedido que ya fue tramitado (incluyendo el caso donde se tramitó gracias a la excepción del `MontoAplazable`) no debe volver a ofrecer la acción "Tramitar Pedido". Cuando llegue el pago que cubre el faltante, la operación correcta es únicamente generar el Complemento de Pago contra la factura existente (regla de arriba) — el Pedido no se vuelve a tramitar, evitando procesar dos veces la misma transición.
...
### 7.2.1 Excepción — `MontoAplazable`

**Existe una regla de negocio que modifica la comparación anterior:** se permite tramitar el pedido aunque falte un monto pequeño por cubrir, sin condonarlo — el faltante queda pendiente de cobro, no se perdona.

**La regla se evalúa sobre el total de la operación de `Validar Cobro`, no por cada Proforma/Factura individual.** Si en una misma operación se cubren varias Proformas/Facturas a la vez, lo que importa es el faltante agregado de toda la operación, no cuánto le falta a cada una por separado.

```
SUM(Proformas/Facturas seleccionadas) − SUM(Cobros + NC seleccionados) <= MontoAplazable
   → se permite tramitar todos los pedidos asociados a esa operación
```

**Ejemplo:** una operación cubre 3 facturas ($500 + $300 + $200 = $1,000 en total), con un Cobro de $950. El faltante de la operación completa es $50 ($1,000 − $950), dentro del límite de $100 — se permite tramitar los 3 pedidos. Si el Cobro hubiera sido de $850 (faltante de $150), no se permite tramitar ninguno de los 3, sin importar cómo estuviera repartido ese faltante entre las 3 facturas.

Parámetros ya definidos:
- `MontoAplazable` es configurable, valor por default $100 MXN.
- Se evalúa por operación completa, no por Proforma/Factura individual (confirmado con el cliente).
- **Pendiente de confirmar con el cliente:** si existe un tope acumulado por cliente a lo largo del tiempo.
- **Pendiente de confirmar con el cliente:** cuando la operación no está en MXN, se propone convertir los $100 MXN a la moneda de la operación usando el `TipoCambio` del propio Cobro recibido (mismo tipo de cambio que ya se registra para ese Cobro, sección 8) — ver documento de preguntas al cliente.

**Mecanismo de implementación:** el Pedido se tramita, pero la Factura **nunca se marca como cobrada artificialmente** — nace en PPD con su `Saldo Insoluto` real (el faltante), y sigue apareciendo normalmente en `Validar Cobro` con `EstatusCobranza=PARCIALMENTE_COBRADA` hasta que el cliente complete el pago. Esto evita el problema de perder la trazabilidad del dinero pendiente y de dejar la factura abierta ante el SAT sin ningún mecanismo para saldarla después.

**Cómo se resuelve el pago posterior, sin duplicar nada:** cuando el cliente cubre el faltante, esa operación cae directamente en la regla ya generalizada de la sección 7.1 ("¿ya existe una factura para este pedido?") — el sistema genera un **nuevo Complemento de Pago** contra la factura existente, por el monto del faltante, y **no vuelve a tramitar el Pedido** (validación explícita en 7.1). No se necesita ningún mecanismo adicional a los ya definidos en esta guía — la clave es que la verificación de "factura existente" en `Validar Cobro` no esté limitada al caso de Prepago con adelantada.

**Este mecanismo está pendiente de aprobación por el cliente** (ver documento de preguntas al cliente) — no de definición técnica, que ya queda resuelta aquí.
...
**Regla de atomicidad — todo o nada:** cuando una operación de `Validar Cobro` requiere generar más de un Complemento de Pago (por el corte de mes), la operación completa debe ser **atómica**. Si el timbrado de cualquiera de los Complementos de Pago involucrados falla, **ninguno** de los Complementos de esa operación queda guardado ni afecta el `Saldo Insoluto` de ninguna Factura — se revierte todo y la operación se marca como fallida en su totalidad. No debe quedar, por ejemplo, el Complemento de noviembre timbrado con la Factura A en `Saldo Insoluto=$100` de forma persistente mientras el de diciembre falla — ese estado intermedio nunca debe sobrevivir fuera de la misma transacción.
...
### 7.8 Catálogo de estatus de Factura — 2 campos independientes, mismo modelo que la guía de NC
...
```sql
INSERT INTO dbo.catFacturaEstatusCobranza (Clave, Descripcion, Orden) VALUES
...
-- EstatusSAT no necesita tabla propia — son solo 2 valores fijos: 'Vigente' / 'Cancelado'
-- (mismo patrón que la guía de NC, sección 13; idealmente ni se guarda, se consulta en vivo al SAT)
```

**`CANCELACION_SOLICITADA` no es un valor de `EstatusCobranza` ni de `EstatusSAT` — es un tercer estado transitorio, propio del flujo de cancelación (ver más abajo), que vive mientras se espera la confirmación.** Se modela como un campo aparte (`Factura.SolicitudCancelacionPendiente`, booleano) para no ensuciar ninguno de los 2 catálogos anteriores con un estado que no es ni "cobranza" ni el resultado final del SAT.

**`CON_INCONSISTENCIA` no aplica aquí — es un estatus de `Cobro` (`catCobroEstatus`), no de `Factura`.** El Cobro es el que se marca con inconsistencia; la Factura no necesita ese valor.

**No hace falta un estatus adicional para cuando una NC cubre la Factura.** Sea que se pague con Cobro, con NC, o con ambos, la transición de `EstatusCobranza` es la misma (`PENDIENTE_COBRO` → `PARCIALMENTE_COBRADA` → `COBRADA`) — el **cómo** ya queda documentado aparte, en los nodos `Pago` del Complemento de Pago y en `ConsumoSaldoAFavor` (guía de NC). Agregar un estatus tipo "Cobrada con NC" duplicaría información que ya vive en otro lado.
...
**Motivo de negocio:** el cliente de Prepago no pagó y no va a pagar. Dado que nunca se permite cobro parcial (regla dura, sección 7.2), la Factura en este escenario solo puede estar en `EstatusCobranza=PENDIENTE_COBRO` (nada pagado) o `COBRADA` (todo pagado) — la cancelación por impago solo aplica sobre `PENDIENTE_COBRO`.

**El Pedido manda — es una sola acción con 2 efectos, sin rama de reversión:**

```
Botón "Cancelar" en Validar Cobro
(solo visible/habilitado si Factura.EstatusCobranza = PENDIENTE_COBRO):

   → Pedido.Estatus = CANCELADO (catálogo propio del Pedido)
   → Se envía la solicitud de cancelación al PAC/SAT
   → Factura.SolicitudCancelacionPendiente = 1 (estado transitorio, no toca EstatusSAT todavía)
```
...
```
→ Factura.EstatusSAT = Cancelado (una vez confirmado en el SAT/PAC)
→ Factura.SolicitudCancelacionPendiente = 0
```
...
**Por qué no comparten catálogo:** en cuanto la Proforma se convierte en Factura (misma acción de `Validar Cobro`, sección 7.1), el trabajo de la Proforma termina — todo el tracking de cobranza pasa a vivir exclusivamente en `catFacturaEstatusCobranza`. La Proforma solo necesita saber que ya se convirtió, y a cuál Factura.
...
**Confirmado — un Cobro sí puede cubrir una Proforma/Factura en una moneda distinta a la suya.** Es un requisito explícito del cliente, y es justo la razón por la que cada objeto guarda su propio `TipoCambio`/`FactorUSD` (Pedido, Factura, Complemento de Pago, Cobro, y también la Nota de Crédito — guía de NC, sección 8): para poder convertir correctamente cuando las monedas no coinciden. El mismo cruce de monedas también está permitido para el saldo a favor de una NC, usando su propio `TipoCambio`/`FactorUSD` heredado (guía de NC, sección 8) en vez del tipo de cambio del día en que se aplica.
...
- [ ] La verificación de "¿ya existe una factura para este pedido?" en `Validar Cobro` es universal — aplica igual si la factura ya existe por Prepago con adelantada, o porque nació antes bajo la excepción del `MontoAplazable` (sección 7.1).
- [ ] Un Pedido ya tramitado nunca vuelve a ofrecer la acción "Tramitar Pedido" — evita procesar dos veces la misma transición cuando llega el pago que cubre un `MontoAplazable` pendiente (sección 7.1).
- [ ] `MontoAplazable`: el Pedido se tramita, pero la Factura nunca se marca como cobrada artificialmente — nace en PPD con su `Saldo Insoluto` real y sigue visible en `Validar Cobro` hasta completarse (sección 7.2.1). Este mecanismo está pendiente de aprobación del cliente, no de definición técnica.
- [ ] `MontoAplazable` se evalúa sobre el faltante agregado de toda la operación de `Validar Cobro` (suma de todas las Proformas/Facturas seleccionadas menos suma de fondos), no por cada Proforma/Factura individual (sección 7.2.1).
- [ ] ⚠️ **PENDIENTE — no resuelto:** confirmar con el cliente si el `MontoAplazable` (100 MXN) se convierte a la moneda de la operación usando el `TipoCambio` del Cobro recibido, cuando la operación no está en MXN (sección 7.2.1).
...
- [ ] `EstatusSAT` y `EstatusCobranza` de la Factura se modelan como 2 campos independientes, igual que en la guía de NC — nunca combinados en un solo valor (sección 7.8).
- [ ] `CON_INCONSISTENCIA` es un estatus de `catCobroEstatus`, no de `catFacturaEstatusCobranza` — no duplicarlo en el catálogo de Factura.
...
- [ ] Cancelar dispara 2 efectos en una sola acción: `Pedido.Estatus = CANCELADO` y `Factura.SolicitudCancelacionPendiente = 1` — el sistema solicita, no rastrea el resultado ante el SAT/PAC (eso lo confirma manualmente el usuario después, actualizando `Factura.EstatusSAT`).
...
- [ ] `catProformaEstatus` es un catálogo independiente de `catFacturaEstatusCobranza`, relacionado 1 a 1 vía `Proforma.FacturaId` — no comparten valores ni se fusionan (sección 7.8).
...
- [ ] ⚠️ **PENDIENTE — no resuelto:** confirmar con el cliente cómo (o si) se sincroniza el estatus de cobro de Legacy (SAP) hacia `catFacturaEstatusCobranza` en ProquifaNet 2 para Crédito/Contra Entrega adelantado — sin esto, esas Facturas quedarían indefinidamente en `PENDIENTE_COBRO` aunque ya se hayan cobrado en Legacy (sección 4).
- [ ] Cuando una operación de `Validar Cobro` genera más de un Complemento de Pago (corte por mes, sección 7.5), el timbrado de todos debe ser atómico — si alguno falla, ninguno queda guardado ni afecta el `Saldo Insoluto` de ninguna Factura.
...
- [ ] Un Cobro sí puede cubrir una Proforma/Factura en moneda distinta — el algoritmo de reparto (sección 7.4) debe soportar esta conversión, no bloquearla. El mismo cruce está permitido para el saldo a favor de NC (usando su propio `TipoCambio`/`FactorUSD`, guía de NC sección 8), ya no hay diferencia entre ambos.
