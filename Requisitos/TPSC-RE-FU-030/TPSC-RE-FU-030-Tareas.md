...
>
> **Dependencias externas:** TPSC-RE-FU-028 completo (`catTipoCFDI.COMPLEMENTO_PAGO`, `fccDocumentoFiscalCobro` con `IdCFDIGeneradaComplemento`, `CFDIGenerada` con `IdCFDIRelacionado`, `catMetodoDePagoCFDI`). TPSC-RE-FU-029 completo (`catTipoCFDI.IdRegion`, `vfccDocumentoFiscalCobro` v2.0). TPSC-RE-FU-021 completo (`FacturaMexicoPdfMappingService`, `PersistirFacturaMexicoPdfService` — patrón base de los nuevos servicios PDF del CP).
>
> **Brechas activas sin bloqueante:** B1 (hora FechaPago) y B5 (formato folio Serie P) son pendientes de bajo impacto que se documentan en código con TODO sin bloquear la implementación. B2 (plantilla correo) bloquea solo la configuración de la plantilla Brevo; el despacho de adjuntos sí puede implementarse. B3 (política reintento CP) se implementa como "línea queda en PENDIENTE" hasta que PMO defina el flujo formal. **No hay brecha bloqueante en este requisito.**
...
| 8   | SERV-TRANSACT        | Implementar ComplementoPagoPdfMappingService y PersistirComplementoPagoPdfService (MinIO cobranza) | Back | ProquifaDotNet.Finanzas |
...
**Resultado esperado:**
`catFormaPagoSAT` existe en ProquifaDotNet con las 22 claves del c_FormaPago SAT, listo para ser referenciado como FK en `fccDocumentoFiscalCobro` y consultado por Finanzas al armar el `ComplementoPagoRequest`.
...
- `catFormaPagoSAT` existe con la estructura definida en `TPSC-RE-FU-030_BD.md`.
...
Ver sección *"Catálogo: catFormaPagoSAT"* en `TPSC-RE-FU-030_BD.md` y sección *"Parte A / A1"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030_BD.md` — DDL + DML catFormaPagoSAT
- `TPSC-RE-FU-030-Back.md` — Parte A, sección A1
...
**Más información de la tarea:**
Ver sección *"DML catUsoCFDI — INSERT clave CP01 (Pagos)"* en `TPSC-RE-FU-030_BD.md` y sección *"Parte A / A2"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030_BD.md` — DML catUsoCFDI
...
**Más información de la tarea:**
Ver sección *"ALTER TABLE fccDocumentoFiscalCobro"* en `TPSC-RE-FU-030_BD.md` y sección *"Parte A / A3"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030_BD.md` — ALTER TABLE fccDocumentoFiscalCobro
- `TPSC-RE-FU-030-Back.md` — Parte A, sección A3
...
**Más información de la tarea:**
Ver sección *"DML EmpresaFolio"* en `TPSC-RE-FU-030_BD.md` y sección *"Parte A / A4"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030_BD.md` — DML EmpresaFolio
...
**Objetivo general:**
Registrar los 4 templates del Complemento de Pago México en `DocumentBuilder.DocumentTemplate` para que `PersistirComplementoPagoPdfService` pueda resolver el `TemplateKey` correcto por empresa emisora al generar el PDF.
...
**Resultado esperado:**
`DocumentBuilder.DocumentTemplate` contiene los 4 templates CP México. `PersistirComplementoPagoPdfService` puede invocar DocumentBuilder con `TemplateKey={Prefijo}_MEX_CP`.
...
**Más información de la tarea:**
Ver sección *"DML DocumentTemplate"* en `TPSC-RE-FU-030_BD.md` y sección *"Parte A / A5"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030_BD.md` — DML DocumentTemplate
- `TPSC-RE-FU-030-Back.md` — Parte A, sección A5
...
- El script v3.0 debe incluir la definición COMPLETA de v2.0 más los agregados de RE-030. Referencia v2.0: `TPSC-RE-FU-029_BD.md`.
...
-- Ejecutar en ProquifaDotNet
-- El script completo debe incluir la definición COMPLETA de v2.0 (TPSC-RE-FU-029_BD.md)
...
**Más información de la tarea:**
Ver sección *"ALTER VIEW vfccDocumentoFiscalCobro v3.0"* en `TPSC-RE-FU-030_BD.md` y sección *"Parte A / A6"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030_BD.md` — ALTER VIEW v3.0
- `TPSC-RE-FU-029_BD.md` — ALTER VIEW v2.0 (base para el script completo)
- `TPSC-RE-FU-030-Back.md` — Parte A, sección A6
...
- Datos del `ComplementoPagoRequest` que llegan desde Finanzas: todos los valores ya calculados (NumParcialidad, ImpSaldoAnt, ImpPagado, ImpSaldoInsoluto, EquivalenciaDR, FechaPago, IdCatFormaPagoSAT, ImpuestosDR si aplica).
...
**Más información de la tarea:**
Ver sección *"Parte C / C1"* en `TPSC-RE-FU-030-Back.md` y criterios B1–G3 en `TPSC-RE-FU-030.md`.

**Recursos:**
- `TPSC-RE-FU-030-Back.md` — Parte C, sección C1
- `TPSC-RE-FU-030.md` — Secciones B, D, E, F, G (criterios XML)
- `TPSC-RE-FU-028-Back.md` — Parte C (patrón base de timbrado)

---

## TAREA 8

**[ RE-FU-030 ] [SERV-TRANSACT] Implementar ComplementoPagoPdfMappingService y PersistirComplementoPagoPdfService (MinIO cobranza)**
...
**Consideraciones previas:**
- Patrón de referencia: `FacturaMexicoPdfMappingService` y `PersistirFacturaMexicoPdfService` de RE-021. Seguir la misma estructura.
- `ComplementoPagoPdfMappingService` tiene dos modos: `MapearPreviewAsync()` (sin sello digital, NumParcialidad estimado) y `MapearAsync()` (con `TimbreFiscalDigital`, QR de verificación SAT).
- `PersistirComplementoPagoPdfService` sube el PDF **y** el XML a MinIO bucket `cobranza` (MEX), resolviendo el bucket a través de `RegionConfiguracionMinioBucket` (BucketClave=`cobranza`, Región=MEX).
...
**Objetivos específicos:**
- Implementar `ComplementoPagoPdfModel` con todas las secciones del PDF (Emisor, Receptor, Comprobante, Concepto fijo, Totales CP, Pago, DoctoRelacionado, ImpuestosDR, Sellos, QR).
- Implementar `ComplementoPagoPdfMappingService.MapearPreviewAsync()` sin `TimbreFiscalDigital`.
- Implementar `ComplementoPagoPdfMappingService.MapearAsync()` con `TimbreFiscalDigital` y QR SAT.
- Implementar `PersistirComplementoPagoPdfService.PersistirAsync()`:
...
**Entregables:**
- Clase `ComplementoPagoPdfModel` con todas las secciones requeridas
- Clase `ComplementoPagoPdfMappingService` (Preview + Async)
- Clase `PersistirComplementoPagoPdfService` (DocumentBuilder + MinIO + Archivo)
...
**Más información de la tarea:**
Ver sección *"Parte B / B6"* y *"Parte E"* en `TPSC-RE-FU-030-Back.md` y criterios I4–I14 en `TPSC-RE-FU-030.md`.

**Recursos:**
- `TPSC-RE-FU-030-Back.md` — Parte B sección B6; Parte E (MinIO)
- `TPSC-RE-FU-030.md` — Sección I (criterios PDF)
- `TPSC-RE-FU-030_BD.md` — Sección MinIO (bucket cobranza)
...
**Consideraciones previas:**
- RE-028 B3 (paso 3) documentó explícitamente: *"Para líneas COMPLEMENTO_PAGO: la previsualización del PDF del Complemento se implementa en TPSC-RE-FU-030"*. Esta tarea cierra ese stub.
- **Prerrequisito:** Tarea 8 (`ComplementoPagoPdfMappingService.MapearPreviewAsync()`) debe estar implementada.
...
- Invocar `ComplementoPagoPdfMappingService.MapearPreviewAsync()`.
...
**Más información de la tarea:**
Ver sección *"Parte B / B1"* en `TPSC-RE-FU-030-Back.md`.

**Recursos:**
- `TPSC-RE-FU-030-Back.md` — Parte B, sección B1
- `TPSC-RE-FU-028-Back.md` — Parte B, sección B3 (contexto del stub)
...
- **Prerrequisitos:** Tareas 1, 3 y 6 (BD) + Tarea 7 (endpoint timbrado CP en Timbrado) + Tarea 8 (PersistirComplementoPagoPdfService).
...
- **Escenario D diferencias:** `IdCFDIRelacionado` = UUID de la FAA (`tpProformaAdelanto.IdCFDIGenerada`); `NumParcialidad = 1` fijo; `ImpSaldoAnt` = Total de la FAA.
...
**Objetivos específicos:**
- Implementar `ComplementoPagoCalculoService`:
...
- Implementar `GenerarComplementoPagoService.GenerarEnCascadaPPDAsync()` (Escenario B):
  - Calcula valores DR.
  - Arma `ComplementoPagoRequest` y llama a Timbrado API.
  - Invoca `PersistirComplementoPagoPdfService`.
...
- Implementar `GenerarComplementoPagoService.GenerarDesdeFacturaAdelantoAsync()` (Escenario D):
...
**Más información de la tarea:**
Ver secciones *"Parte B / B2, B3, B4"* en `TPSC-RE-FU-030-Back.md`. Contexto de Escenarios B y D en `TPSC-RE-FU-028-Back.md` sección B4.

**Recursos:**
- `TPSC-RE-FU-030-Back.md` — Parte B, secciones B2, B3, B4
- `TPSC-RE-FU-030.md` — Reglas de Negocio 2, 7, 8, 9, 10, 11; Criterios D–G
- `TPSC-RE-FU-028-Back.md` — Parte B, sección B4 (contexto cascada PPD Escenarios B y D)
...
**Más información de la tarea:**
Ver sección *"Parte D"* en `TPSC-RE-FU-030-Back.md` y criterios I4–I14 en `TPSC-RE-FU-030.md`.

**Recursos:**
- `TPSC-RE-FU-030-Back.md` — Parte D
- `TPSC-RE-FU-030.md` — Sección I, criterios I4–I14
