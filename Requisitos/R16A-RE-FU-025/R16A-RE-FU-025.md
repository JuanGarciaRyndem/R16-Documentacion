...
**Regla 7 — Medio de pago regionalizado: NO obligatorio en Perú, catálogo RESUELTO (DUDA-076)**
...
**Resolución DUDA-076 (cerrada):** el campo "Forma de Pago"/"Medio de pago" se **regionaliza**: es **obligatorio únicamente para México**; en Perú (05/08) **deja de ser obligatorio** — SUNAT no exige declarar el medio de pago específico en el comprobante, por lo que en Perú el dato pasa a ser opcional/informativo para Tesorería, no un requisito de captura. El 18/08 el cliente compartió la lista definitiva de medios de pago válidos para Perú, inicialmente entregada como imagen. En consecuencia:
- (a) el campo queda regionalizado (obligatorio solo en México);
- (b) para Perú **no es obligatorio**;
- (c) ~~su transcripción a valores discretos del catálogo (catMedioDePago) sigue pendiente~~ **✅ RESUELTO (2026-08-24): el cliente confirmó que el catálogo ya se encuentra actualizado/cargado en `catMedioDePago`**, con 4 valores para Perú: Depósito en cuenta (`001`), Transferencia de fondos (`003`), Tarjeta de débito (`005`), Otros medios de pago (`999`). Los 4 registros tienen `ObligatorioEnCliente = 0`, lo que confirma en BD la no-obligatoriedad resuelta por esta duda. Detalle completo (valores, GUIDs, mapeo de columnas) en `R16A-RE-FU-025_BD.md`.
...
**Riesgo 1 — ~~Catálogo de valores de medio de pago para Perú pendiente de transcripción~~ RESUELTO (DUDA-076, 2026-08-24)**
DUDA-076 (cerrada) resolvió que el campo se regionaliza y deja de ser obligatorio para Perú. El 2026-08-24 el cliente confirmó que el catálogo definitivo ya se encuentra cargado en `catMedioDePago` (4 valores: Depósito en cuenta, Transferencia de fondos, Tarjeta de débito, Otros medios de pago). Riesgo cerrado; sin remanente pendiente sobre este punto.
...
**Criterio D4 — Medio de pago (catálogo interno, NO obligatorio en Perú — DUDA-076 RESUELTA)**
Dado que el usuario captura el cobro,
Cuando el sistema renderiza el campo,
Entonces deberá ofrecer un combo **NO obligatorio** (regionalización DUDA-076: obligatorio solo en México) con el catálogo interno de medio de pago Perú. SUNAT no exige este dato fiscalmente. **✅ Catálogo definitivo cargado en BD (2026-08-24):** el combo debe presentar los 4 valores confirmados en `catMedioDePago` para la región Perú: Depósito en cuenta (clave `001`), Transferencia de fondos (clave `003`), Tarjeta de débito (clave `005`) y Otros medios de pago (clave `999`), todos activos. Detalle de GUIDs y columnas en `R16A-RE-FU-025_BD.md`.
...
- Diferencia — Medio de pago: en México se usa el catálogo SAT c_FormaPago y el campo es obligatorio; en Perú, por resolución de **DUDA-076 (cerrada)**, el campo se regionaliza y **deja de ser obligatorio**. ~~El cliente entregó el 18/08 la lista definitiva de valores para Perú, pero como imagen. Pendiente: transcribir el catálogo definitivo Perú desde la imagen (18/08) y cargarlo en catMedioDePago; no inventar valores mientras tanto.~~ **✅ Actualización 2026-08-24:** el catálogo definitivo ya fue cargado por el cliente en `catMedioDePago` (4 valores para la región Perú: Depósito en cuenta `001`, Transferencia de fondos `003`, Tarjeta de débito `005`, Otros medios de pago `999`; todos con `ObligatorioEnCliente = 0`). Ver detalle completo, incluyendo nota sobre el mapeo de columnas inferido y la corrección de que `ClaveFormaDePago` sí tiene valores en Perú (no es NULL como se asumía antes), en `R16A-RE-FU-025_BD.md` y `R16A-RE-FU-025-Back.md`.
- **Trazabilidad (2026-08-21):** actualización de este documento conforme a la resolución de DUDA-076 — "Medio de pago" regionalizado (obligatorio solo México, no obligatorio en Perú); catálogo Perú entregado por el cliente el 18/08 como imagen, transcripción de valores pendiente en ese momento.
- **Trazabilidad (2026-08-24):** el cliente entregó los valores definitivos del catálogo `catMedioDePago` para Perú (ya transcritos/cargados en el sistema, no más como imagen), cerrando la Brecha B2 de DUDA-076. Se actualizaron en consecuencia Regla 7, Alcance ("No aplica a"), Riesgo 1, Criterio D4 y los archivos de impacto `_BD.md` y `-Back.md` de este requisito.
