...
- El folio del cobro tiene formato COB-mmddaa-consecutivo (confirmado por PMO #40), donde la fecha corresponde al día efectivo del cobro (no a la fecha de llegada del correo al Buzón). **Identificación del correo antes de la captura (resuelta):** el listado identifica el correo mediante los datos del Buzón (fecha de llegada, remitente, asunto y `IdCorreoRecibidoCliente` como identificador temporal); el **folio definitivo COB-mmddaa-consecutivo se genera únicamente al confirmar la captura**. Comportamiento ya especificado en la matriz del requisito.
...
- **Decisión OBS-050 (confirmada) — Doble Tipo de Cambio (fiscal + operativo):** el cobro persiste **dos tipos de cambio** para cubrir los dos usos: (a) **TC vs MXN** — para el CFDI/Complemento de Pago (obligación fiscal SAT, moneda base MXN); (b) **TC vs Moneda de Facturación del cliente** — para asociar/cubrir el cobro contra facturas o proformas emitidas en esa moneda (uso operativo del Paso 2). Ambos se pre-cargan con el criterio DOF de OBS-049 y son editables. Cuando la moneda de facturación del cliente es MXN, el segundo TC no aplica.
...
- **Resuelto — identificación visual del correo antes de captura:** el listado muestra los datos del correo del Buzón (fecha de llegada, remitente, asunto) y el folio definitivo COB-mmddaa-consecutivo se genera al confirmar la captura (no hay folio provisional). Especificado en la matriz del requisito.
