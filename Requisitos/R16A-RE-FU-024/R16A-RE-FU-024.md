...
| **Historia de Usuario** | Yo como **Gestor de Cobranza**, quiero contar con la primera pantalla del wizard de Validar Cobro (Paso 1 - Captura del Cobro) para revisar los correos del Buzón de Cobros del cliente y capturar los datos del cobro recibido o marcar sus inconsistencias, para registrar formalmente cada cobro y prepararlo para su asociación en el Paso 2. |
| **Prioridad** | Alta |
| **Estado** | Propuesto |
...
- Formulario de Datos del Cobro: folio del cobro (~~formato COB-mmddaa-consecutivo~~ **formato COM-mmddaa-consecutivo para México — DUDA-072, ver Regla 6 y Nota**, donde la fecha corresponde al día efectivo del cobro), monto recibido, fecha del cobro, forma de pago (catálogo SAT c_FormaPago), cuenta origen (texto libre), cuenta destino (Catálogo de Cuentas Bancarias del grupo PROQUIFA México), moneda del cobro, tipo de cambio del día (vs moneda de facturación), notas del cobro.
- Asistencia automatizada (DUDA-070 — Resuelta): el sistema incluye, como alcance comprometido de R16, el auto-completado de los campos del formulario a partir del análisis del correo y del comprobante de pago seleccionado. El detalle técnico de implementación se define en construcción.
...
- ~~Detalle técnico de la asistencia automatizada propuesta (modelo de IA, motor de extracción de datos, integraciones técnicas): pendiente de definición, no está comprometido como alcance R16.~~ **(DUDA-070 — Resuelta):** la funcionalidad SÍ está comprometida como alcance R16; solo el detalle técnico de implementación (modelo, motor de extracción, integraciones) queda para fase de construcción.
...
El formulario de captura del cobro ofrece los siguientes campos: Folio del cobro (~~COB-mmddaa-consecutivo~~ **COM-mmddaa-consecutivo — DUDA-072 Resuelta**: prefijo "COM" (CO=Cobro, M=México), consecutivo independiente por región — Perú usa prefijo "COP" con su propio consecutivo, ej. "COM-071726-1" / "COP-071726-1"; los prefijos deben ser intuitivos en pantalla, mostrando su significado —, generado por el sistema con la fecha efectiva del cobro); Monto recibido (obligatorio); Fecha del cobro (obligatorio, datepicker; corresponde al día efectivo del cobro); Forma de pago (obligatorio, combo del catálogo SAT c_FormaPago: ejemplo "01 - Efectivo", "02 - Cheque nominativo", "03 - Transferencia electrónica de fondos", etc.); Cuenta origen (obligatorio, texto libre, es la cuenta del cliente); Cuenta destino (obligatorio, combo del Catálogo de Cuentas Bancarias del grupo PROQUIFA México); Moneda del cobro (obligatorio, combo); Tipo de Cambio (calculado automáticamente con el TC del día vs la moneda de facturación del cliente, no modificable por el usuario); Notas del cobro (opcional, texto libre).
...
**Regla 8 — Asistencia automatizada para captura del cobro (DUDA-070 — Resuelta)**
Sobre un correo del Buzón con su adjunto seleccionado como comprobante de pago, el sistema ofrece asistencia automatizada, **incluida y comprometida como alcance de R16**, para proponer valores de auto-completado de los campos del formulario (monto, fecha, forma de pago, cuenta origen, moneda) extraídos del contenido del correo y del comprobante. El usuario siempre tiene la última palabra sobre los valores capturados; los datos sugeridos son editables antes de confirmar. ~~El alcance técnico de esta asistencia (modelo de IA, motor de extracción, integraciones, precisión esperada, comportamiento ante baja confianza) queda pendiente de definición y NO está comprometido como alcance R16.~~ El detalle técnico de implementación (modelo, motor de extracción, integraciones, precisión esperada, comportamiento ante baja confianza) se define en fase de construcción.
...
Entonces deberá mostrar el folio ~~formato COB-mmddaa-consecutivo~~ **formato COM-mmddaa-consecutivo (DUDA-072 — Resuelta)**, donde la fecha se construye con el día efectivo del cobro (campo Fecha del cobro del formulario). El folio se genera al confirmar la captura del cobro. ~~Pendiente confirmar si el folio del cobro es por región (consecutivo independiente para México y Perú) o global.~~ **Confirmado: el consecutivo es independiente por región. México usa prefijo "COM" (CO=Cobro, M=México); Perú usa prefijo "COP" (CO=Cobro, P=Perú); mismo formato mmddaa-consecutivo en ambos casos, ej. "COM-071726-1" / "COP-071726-1". Prefijos intuitivos (muestran su significado en pantalla).**
...
**Criterio D8 — Tipo de cambio del día capturado con el cobro (OBS-049 — Resuelta)**
Dado que el usuario selecciona la moneda del cobro,
Cuando el sistema renderiza el campo Tipo de Cambio,
Entonces deberá comportarse según el caso: si la moneda del cobro es MXN y la moneda de facturación del cliente es MXN, el campo se muestra como N/A; si la moneda del cobro es MXN y la moneda de facturación del cliente es distinta a MXN, el sistema pre-carga automáticamente el TC de la moneda de facturación del cliente respecto a MXN, capturado anticipadamente para conversiones operativas en el Paso 2 con documentos en esa moneda; si la moneda del cobro es distinta a MXN, el sistema pre-carga automáticamente el TC de la moneda del cobro respecto a MXN.
**Fuente y criterio del TC (OBS-049):** el sistema utilizará el **Tipo de Cambio Oficial DOF (FIX Banxico)** correspondiente a la **fecha del comprobante de pago** (capturada por el Gestor en el campo "Fecha del cobro"), usando el último valor publicado si es día inhábil. **No se aplica el margen comercial del 2.5% (Valor de Venta)** — el margen ya está incorporado en el precio de la cotización y aplicarlo nuevamente generaría déficit sistemático contra el cliente (≈ $437 MXN en 1,000 USD) superando la tolerancia operativa de $100 MXN. El TC Oficial es neutral, verificable por el cliente y coincide con el criterio fiscal del SAT para el complemento de pagos (CP).
**Editabilidad:** el campo TC pre-cargado es **editable** por el usuario (no bloquea el avance del Paso 1). Cualquier modificación manual queda registrada con el cobro. El TC capturado se conserva y se utiliza en el Paso 2 (conversiones operativas) y en el Paso 3 (TipoCambio del CFDI fiscal cuando aplique).
...
**Criterio E1 — Auto-completado del formulario (DUDA-070 — Resuelta)**
...
Entonces deberá ofrecer asistencia automatizada, **incluida y comprometida como alcance de R16**, para auto-completar los campos del formulario: monto recibido, fecha del cobro, forma de pago, cuenta origen, moneda. ~~El alcance técnico de esta asistencia (motor de extracción, modelo IA, precisión esperada, integraciones) queda pendiente de definición y no está comprometido como alcance R16.~~ El detalle técnico (motor de extracción, modelo IA, precisión esperada, integraciones) se define en fase de construcción.
...
- El folio del cobro tiene ~~formato COB-mmddaa-consecutivo (confirmado por PMO #40)~~ **formato COM-mmddaa-consecutivo para México (DUDA-072 — Resuelta; prefijo "COM" = Cobro México, consecutivo independiente por región; Perú usa "COP")**, donde la fecha corresponde al día efectivo del cobro (no a la fecha de llegada del correo al Buzón). **Identificación del correo antes de la captura (DUDA-075 — Resuelta, ya definida en la matriz del requisito):** el listado identifica el correo mediante los datos del Buzón (fecha de llegada, remitente, asunto y `IdCorreoRecibidoCliente` como identificador temporal); el **folio definitivo COM-mmddaa-consecutivo se genera únicamente al confirmar la captura**. Comportamiento ya especificado en la matriz del requisito (Regla 3, Criterio B1) — verificado, no requiere cambios adicionales.
...
- **Decisión OBS-050 (confirmada; ver también DUDA-074, misma resolución) — Doble Tipo de Cambio (fiscal + operativo):** el cobro persiste **dos tipos de cambio** para cubrir los dos usos: (a) **TC vs MXN** — para el CFDI/Complemento de Pago (obligación fiscal SAT, moneda base MXN); (b) **TC vs Moneda de Facturación del cliente** — para asociar/cubrir el cobro contra facturas o proformas emitidas en esa moneda (uso operativo del Paso 2). Ambos se pre-cargan con el criterio DOF de OBS-049 y son editables. Cuando la moneda de facturación del cliente es MXN, el segundo TC no aplica.
- ~~Asistencia automatizada propuesta: funcionalidad exploratoria para auto-completar los datos del formulario a partir del correo y el comprobante. No está comprometida como alcance R16; el alcance técnico queda pendiente de definición.~~ **(DUDA-070 — Resuelta):** la asistencia automatizada para auto-completar los datos del formulario a partir del correo y el comprobante SÍ está incluida y comprometida como alcance de R16; el detalle técnico de implementación se define en construcción. El usuario siempre tiene la última palabra sobre los valores capturados.
...
- ~~Pendiente confirmar si el folio del cobro (COB-mmddaa-consecutivo) es por región (consecutivo independiente para México y Perú) o global del grupo.~~ **(DUDA-072 — Resuelta):** el consecutivo es independiente por región. México: prefijo "COM" (CO=Cobro, M=México), formato COM-mmddaa-consecutivo, ej. "COM-071726-1". Perú: prefijo "COP" (CO=Cobro, P=Perú), mismo formato, ej. "COP-071726-1". Los prefijos deben ser intuitivos en pantalla (mostrar su significado).
- **Resuelto (DUDA-075) — identificación visual del correo antes de captura:** el listado muestra los datos del correo del Buzón (fecha de llegada, remitente, asunto) y el folio definitivo COM-mmddaa-consecutivo se genera al confirmar la captura (no hay folio provisional). Especificado en la matriz del requisito (Regla 3, Criterio B1) — verificado sin cambios adicionales.
...
- ~~Pendiente definir el alcance técnico de la asistencia automatizada propuesta para auto-completar el formulario (modelo de IA o motor de extracción, integración, precisión esperada, comportamiento ante baja confianza). No comprometido como alcance R16.~~ **(DUDA-070 — Resuelta):** la funcionalidad SÍ está comprometida como alcance R16; solo el detalle técnico de implementación (modelo de IA o motor de extracción, integración, precisión esperada, comportamiento ante baja confianza) queda para fase de construcción.
- **Decisión OBS-049 (confirmada; ver también DUDA-073, misma resolución):** el TC del día en el Paso 1 usa el **Tipo de Cambio Oficial DOF (FIX Banxico)** de la fecha del comprobante de pago, **sin margen comercial del 2.5%**. El campo se pre-carga automáticamente pero es **editable** por el Gestor y **no bloquea el avance** del wizard. El sistema almacena localmente dos valores diarios (Oficial y Venta = Oficial + 2.5%); para efectos de validación de cobertura del cobro se utiliza exclusivamente el Oficial. Justificación: alineación fiscal SAT (CP), evita doble aplicación del margen (ya cobrado en la cotización), variaciones menores absorbidas por tolerancia operativa de $100 MXN.
- **Decisión OBS-051 (confirmada) — Fecha del pago cuando se aplica un saldo a favor previo:** cuando un saldo a favor generado previamente (por devolución, nota de crédito o pago excedente) se aplica para extinguir total o parcialmente una nueva factura, la fecha registrada para el movimiento en el Complemento de Recepción de Pagos es la **fecha en que el saldo a favor se aplica** a la nueva factura, **no la fecha en que se originó** dicho saldo. Alineado a la Guía de Llenado CFDI 4.0 / CRP 2.0 del SAT: la fecha de pago corresponde al momento en que se extingue la obligación de pago.
- **Decisión OBS-052 (confirmada con soporte legal y fiscal) — TC del pago y TC para cubrir la factura:**
  - **(a) TC del pago:** se fija con el TC Oficial DOF de la **fecha en que el dinero efectivamente llega al banco de PROQUIFA** (fecha del comprobante), **no** la fecha en que el cliente envía el comprobante al sistema. Criterio ya cerrado en OBS-049.
  - **(b) TC para cubrir la factura/proforma cuando cobro y documento están en monedas distintas:** se utiliza el **TC del pago** (día de llegada al banco), **no** el TC impreso en la factura al emitirse. Ejemplo: factura emitida el viernes por 1,000 USD con TC de emisión 18.00; cliente paga el lunes MXN 17,900 (TC del día 17.90). La factura queda cubierta al 100% (17,900 ÷ 17.90 = 1,000 USD). Usar el TC de emisión (18.00) arrojaría 994.44 USD y un falso saldo pendiente de 5.56 USD.
  - **Fundamento legal:** Art. 8 Ley Monetaria de los EUM — las obligaciones pactadas en moneda extranjera se solventan al TC vigente en la fecha del pago.
  - **Fundamento fiscal:** Guía CFDI 4.0 / CRP 2.0 — la equivalencia y saldo insoluto (`EquivalenciaDR`) se determinan con el TC del pago. La diferencia contra el TC de emisión ($100 MXN en el ejemplo) NO es saldo pendiente del cliente; se registra contablemente como **fluctuación cambiaria** (pérdida o ganancia) de la empresa emisora conforme a LISR y NIF B-15.
...
- **(2026-08-24) — Diseño de la Solución (DIS-SOL) publicado por Osmar Calderón, v1.0 (20/07/2026, revisor Valdemar Farina Sánchez).** Documenta el diseño técnico back-end del Paso 1: la tabla fusionada `fccCobroCliente` (fusión de `fccPagoCliente` + `fccFolioPagoCliente`, DDL completo, ACEPTADA), el catálogo `catCobroEstatus` con 7 estatus finales (BORRADOR, CAPTURADO, ASOCIADO, SALDO_A_FAVOR, CON_INCONSISTENCIA, COMPLETADO, CANCELADO — **sin** estatus `TIMBRADO`, decisión D2b), el retiro de las banderas `Confirmado`/`BloqueadoPorTimbrado`/`FechaBloqueoTimbrado` en favor del estatus como única fuente de verdad, los endpoints nuevos (listado, guardado/edición unificado, finalización con folio, TC del día, inconsistencia, estado del wizard), y el foliador `fccCobroClienteConsecutivo` + `sp_IncrementarFolioCobro` con una fila por región. Ver archivo `[R16A-RE-FU-024][DIS-SOL] Diseño de la solución.md` en esta misma carpeta. **⚠️ Conflicto detectado con DUDA-072:** el diseño (v1.0, fechado 20/07/2026, decisión D8) especifica el folio como **`COB-MMDDYY-NNNN`** (prefijo único "COB", 4 dígitos), mientras que DUDA-072 —resuelta el 21/08/2026, **posterior** a esta versión del diseño— estableció prefijos diferenciados por región: **`COM`** para México y **`COP`** para Perú. El diseño no incorpora todavía este cambio (tampoco actualiza el seed provisional `PER/COB/0` del foliador). Pendiente reconciliar: actualizar el DIS-SOL a `COM-MMDDYY-NNNN` antes de construcción, o confirmar con el cliente/coordinador si DUDA-072 aplica también al nombre técnico del foliador.
- **(2026-08-24) — Otras decisiones técnicas del DIS-SOL relevantes al requisito funcional:** `IdEmpresa` pasa a nullable (D13); se agrega `IdRegion` a `fccCobroCliente`, heredado del cliente, para que el foliador elija la fila correcta por región (D8b); se agrega `IdCorreoRecibidoClienteReferencia` (nullable, índice único filtrado) como llave idempotente contra reintentos de los BOs legacy del Mailbot (D12, coordinación con RE-008); el TC usa `ValorDeCompra` de `TipoDeCambioBanamex` (no `ValorDeVenta` ni la vista `vTipoDeCambioBanamex`/`ConversorDivisas`) — consistente con OBS-049/OBS-050 ya reflejadas en este requisito (D6, D7); marcar inconsistencia ya **no** apaga `Activo` (D2c) — el cobro sigue contando como pendiente hasta resolverse.
...
| 3 | 2026-08-10 | OBS-049 | Fuente y criterio del TC del Paso 1 confirmados: **TC Oficial DOF (FIX Banxico) de la fecha del comprobante**, **sin margen 2.5%**. Campo **editable** por el Gestor sin bloquear el avance. Actualizado: Criterio D8, Notas Adicionales. Cierra brecha "Fuente oficial TC del día para México". |
| 4 | 2026-08-10 | OBS-050 | Doble Tipo de Cambio por cobro: (a) TC vs MXN (fiscal, CFDI/CP) y (b) TC vs moneda de facturación (operativo, asociación a facturas/proformas). Ambos persistidos en `fccPagoCliente`. Cierra la duda abierta sobre moneda base de conversión. |
| 5 | 2026-08-10 | OBS-051 | Fecha del pago cuando se aplica un saldo a favor previo: se registra la **fecha de aplicación** del saldo a la nueva factura, no la fecha en que se originó el saldo. Alineado a CFDI 4.0 / CRP 2.0 del SAT. |
| 6 | 2026-08-10 | OBS-052 | Confirmado con soporte legal (Art. 8 Ley Monetaria EUM) y fiscal (Guía CFDI 4.0 / CRP 2.0): el TC del pago (fecha llegada al banco) es el que se utiliza para calcular la cobertura del cobro contra la factura/proforma, NO el TC de emisión de la factura. La diferencia se registra como fluctuación cambiaria (LISR / NIF B-15). |
| 7 | 2026-08-10 | Duda de diseño | Identificación visual del correo antes de la captura: resuelta con lo ya especificado en la matriz (datos del Buzón; folio COB-mmddaa-consecutivo generado únicamente al confirmar). |
| 8 | 2026-08-21 | DUDA-070 | Asistencia automatizada (IA/motor de extracción) para auto-completar el Paso 1: queda **incluida y comprometida como alcance de R16** (antes "no comprometida, pendiente de definición"). Afecta: Requisito Funcional, Alcance (Aplica a / No aplica a), Regla 8, Criterio E1, Notas Adicionales. El detalle técnico de implementación se resuelve en construcción. |
| 9 | 2026-08-21 | DUDA-072 | Formato de folio actualizado: **COM-mmddaa-consecutivo** para México (CO=Cobro, M=México) y **COP-mmddaa-consecutivo** para Perú, con **consecutivo independiente por región** (ya no COB- global). Prefijos intuitivos en pantalla. Afecta: Alcance, Regla 6, Criterio D1, Notas Adicionales. |
| 10 | 2026-08-21 | DUDA-075 | Verificado: la identificación visual del correo antes de la captura ya estaba correctamente especificada en la matriz (Regla 3, Criterio B1); no requirió cambio, solo se referenció la duda como trazabilidad. |
| 11 | 2026-08-21 | DUDA-047 | Homologada la denominación del rol operativo a "Gestor de Cobranza"; se eliminan las menciones a "Analista de Cuentas por Cobrar" como nombre del rol. |
| 12 | 2026-08-24 | Diseño de la Solución (DIS-SOL) | Se publica el diseño técnico back-end del Paso 1 (Osmar Calderón, v1.0, revisor Valdemar Farina Sánchez): tabla fusionada `fccCobroCliente`, catálogo `catCobroEstatus` (7 estatus, sin `TIMBRADO`), endpoints nuevos, foliador por región. Se detecta y documenta un conflicto pendiente de reconciliar entre el formato de folio del diseño (`COB-`) y el de DUDA-072 (`COM-`/`COP-`, resuelta después de la versión 1.0 del diseño). |
