...
- **Convivencia flags ↔ `IdCatCobroEstatus`** — ✅ catálogo adoptado (ver Decisiones en el contexto), y **16/07/2026 se decide la dirección: el estatus es la fuente de verdad, se retiran flags.** Primer paso ya tomado: `BloqueadoPorTimbrado`/`FechaBloqueoTimbrado` se quitan de la tabla y se reemplazan por un estatus (`TIMBRADO`) — ver *Fusión ACEPTADA → Propuesta: reemplazar `BloqueadoPorTimbrado` por un estatus*. Lo que queda por definir:
  - **La traza temporal** de las transiciones (el flag llevaba `FechaBloqueoTimbrado`; el estatus no guarda *cuándo* — ¿tabla de historial de estatus? ¿fecha por transición?).
  - **`Confirmado` también dropeado** (16/07/2026) — ya no quedan banderas de estado en la tabla, solo `Activo` (soft-delete universal). El estatus es la única fuente de verdad del ciclo.
  - **Coordinación:** el catálogo lo crea **RE-002** y la FK la pone **RE-008**; el alta del nuevo estatus (`TIMBRADO`) va con ellos.
