...
La bandeja del Gestor de Cobranza en los módulos operativos refleja siempre los pendientes y pagos de los clientes que tiene asignados en ese momento. La reasignación de Cobrador de un cliente redistribuye inmediatamente sus pendientes y pagos a la bandeja del nuevo Cobrador; la bandeja del Cobrador anterior deja de mostrarlos. **Punto de corte (DUDA-116, deriva de OBS-005):** el corte entre "pendiente ya trabajado" y "en proceso" es por pantalla/módulo. Un pendiente sigue abierto (y por tanto se reasigna al nuevo Cobrador) mientras aparece sin finalizar en su pantalla; los avances parciales dentro de un flujo de varios pasos (p. ej. seguimiento registrado o un paso guardado del wizard de Factura por Adelantado o de Validar Cobro) no lo cierran ni lo excluyen de la reasignación. Solo lo finalizado en una pantalla permanece atribuido a quien lo trabajó (ver Regla 4 / Criterio C4 en `R16A-RE-FU-002-Back.md`).
...
- **Entonces** el sistema deberá mostrar los pendientes y pagos del cliente en la bandeja del Cobrador B en la siguiente consulta de la bandeja, y dejar de mostrarlos en la bandeja del Cobrador A. ~~La reasignación aplica de manera dinámica sobre todos los pendientes y pagos vigentes del cliente, incluidos los que aún no han sido trabajados: todos pasan a la bandeja del nuevo Cobrador, sin distinción entre los previos a la reasignación y los nuevos.~~ *(Redacción imprecisa: no distinguía el punto de corte por pantalla/módulo. Corregido por DUDA-116.)* La reasignación aplica sobre los pendientes **abiertos** del cliente en cada pantalla/módulo (los que aún no han sido finalizados, incluyendo los que tienen avances parciales guardados, p. ej. un paso de wizard): todos ellos pasan a la bandeja del nuevo Cobrador. Lo ya finalizado en una pantalla no se reasigna (ver Criterio C4).
...
- **Comportamiento del filtro de bandeja: dinámico.** Cuando el Coordinador o el Gerente de Tesorería reasigna el Cobrador de un cliente, los pendientes y pagos **abiertos** del cliente (los que aún no han sido finalizados en su pantalla) se reflejan inmediatamente en la bandeja del nuevo Cobrador y dejan de aparecer en la del Cobrador anterior. Aparte del filtro dinámico de la bandeja, el sistema conserva en el histórico el registro del trabajo ya realizado por el Cobrador anterior, de modo que la reasignación no borra esa trazabilidad.
- **Punto de corte "trabajado" vs. "en proceso" (DUDA-116, deriva de OBS-005):** el corte es por pantalla/módulo. Un pendiente sigue abierto mientras aparece en su pantalla sin finalizarse; los avances parciales (seguimiento registrado, pasos guardados del wizard de Factura por Adelantado o Validar Cobro) no lo cierran. Al reasignar, solo se transfieren los pendientes abiertos de cada pantalla; lo ya finalizado en una pantalla permanece con quien lo trabajó.
...
| 3 | 2026-08-21 | Punto de corte "trabajado" vs. "en proceso" al reasignar Cobrador | Se precisa en Regla 4, Criterio C2 y Notas de Implementación que el corte es por pantalla/módulo: un pendiente sigue abierto (y se reasigna) mientras no se finalice en su pantalla; los avances parciales dentro de un flujo de varios pasos (wizard) no lo cierran. Se tacha la redacción anterior del Criterio C2 por imprecisa. Consistente con lo ya documentado en `R16A-RE-FU-002-Back.md` (OBS-005) y `R16A-RE-FU-002_BD.md`. Referencia: DUDA-116 (deriva de OBS-005). |
