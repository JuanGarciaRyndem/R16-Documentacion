...
* ~~**Factura Perú (RE-FU-022)** — plantillas _PER_FAC y DIS propios; se documenta aparte (hermano regional).~~ **[2026-08-21 · DUDA-052]** Obsoleto: se cancela por completo la facturación de Perú en R16 (no solo se restringe a Prepago); no hay desarrollo de plantillas _PER_FAC ni DIS propio para Perú en este release.
* **Factura Anticipo** (Sección K de la matriz, pedidos con productos controlados) — PROQUIFA aún no la tiene implementada ni existe PDF de referencia confirmado; la matriz solo incluye un esbozo tentativo no confirmado. Queda fuera de alcance hasta que se implemente y el asesor comercial valide su diseño. **[2026-08-21 · DUDA-061 · Descartada]** La duda formal sobre el diseño completo de la Factura Anticipo se descartó (redirigida a DUDA-045); esto NO define un diseño definitivo — sigue pendiente de confirmar con el asesor comercial, sin PDF de referencia.
...
| D2 | Dado que el sistema muestra el campo Referencia de cada cuenta, Cuando construye el valor, Entonces ~~pendiente confirmar si la construcción de la referencia sigue el mismo método que en la Proforma o si tiene reglas propias para la Factura.~~ **[2026-08-21 · DUDA-059 · Resuelta]** deberá usar el mismo método que la Proforma. | Se pinta BankAccount.CustomerRef de forma literal; la construcción del valor (misma lógica Banamex/no-Banamex + Código Validador que la Proforma, confirmado por DUDA-059) es responsabilidad de Back-end. | Cubierto (maqueta agnóstica) | **[2026-08-21 · DUDA-059]** Resuelto: la referencia usa el mismo método que la Proforma (Código Validador), sin reglas propias — no bloquea la maqueta, que solo pinta el valor recibido. |
| D3 | Dado que el sistema muestra la sección, Cuando incluye el dato del pedido, Entonces deberá mostrar el número de orden de compra del cliente o referencia equivalente proveniente del Pedido. ~~Confirmar diferencia con Folio de Pedido Interno.~~ **[2026-08-21 · DUDA-060 · Resuelta]** | Se cubre con Voucher.CustomerOrderNumber, distinto de InternalOrderNumber (2 datos independientes). | Cubierto | **[2026-08-21 · DUDA-060]** Resuelto: son datos distintos y no intercambiables — CustomerOrderNumber es la Referencia capturada en Buzones (orden de compra del cliente); InternalOrderNumber es el Folio de PI generado por PQF2 en la Confirmación de Pedido. Consistente con el diseño previo (2 campos separados en el DTO). |
...
| 1.1 | 21 ago 2026 | (actualización documental) | Actualización por dudas cerradas: DUDA-039 (persistencia de PDF sin regeneración), DUDA-052 (cancelación total de facturación Perú, no solo restricción a Prepago), DUDA-059 (referencia bancaria = método de la Proforma), DUDA-060 (Referencia de Buzones ≠ Folio de PI), DUDA-061 (Factura Anticipo descartada/redirigida a DUDA-045, diseño sigue pendiente) | — |
