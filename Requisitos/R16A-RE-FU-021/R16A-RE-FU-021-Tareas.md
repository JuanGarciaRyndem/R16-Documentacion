...
| 11 | IMP-EXIST-SERVICE | Extender AdvanceInvoiceGenerateService con persistencia real del PDF (pasos 10-11 RE-FU-019 T13) | Back | ProquifaDotNet.Finanzas | — |
| 12 | IMP-EXIST-SERVICE | Extender AdvanceInvoicePreviewService con template real de Factura México (RE-FU-019 T15) | Back | ProquifaDotNet.Finanzas | — |
...
- La tabla existe con la estructura definida en `R16A-RE-FU-021_BD.md`.
...
Ver sección *"Cambios DDL Requeridos — catClaveProdServSAT"* en `R16A-RE-FU-021_BD.md`.

**Recursos:**
- `R16A-RE-FU-021_BD.md` — Script propuesto `CREATE TABLE catClaveProdServSAT`
...
**Más información de la tarea:**
Ver sección *"Cambios DDL Requeridos — ClaveProdServSAT en Producto"* en `R16A-RE-FU-021_BD.md`.

**Recursos:**
- `R16A-RE-FU-021_BD.md` — Scripts propuestos ALTER TABLE Producto
...
**Más información de la tarea:**
Ver sección *"Cambios DDL Requeridos — ClaveSAT en catUnidad"* en `R16A-RE-FU-021_BD.md`.

**Recursos:**
- `R16A-RE-FU-021_BD.md` — Script propuesto ALTER + ejemplos de claves SAT
...
**Más información de la tarea:**
Ver sección *"Cambios DDL Requeridos — Exportacion en CFDIGenerada"* en `R16A-RE-FU-021_BD.md`.

**Recursos:**
- `R16A-RE-FU-021_BD.md` — Script propuesto ALTER TABLE CFDIGenerada
...
**Más información de la tarea:**
Ver criterios A1–J2 en `R16A-RE-FU-021.md`. Patrón de templates en `Scripts/20251001_1439/01_Insert_templates.sql`.

**Recursos:**
- `R16A-RE-FU-021.md` — Criterios A1–J2
...
**Consideraciones previas:**
- El PDF de la Factura CFDI 4.0 integra datos de múltiples tablas: `CFDIGenerada`, `Producto`, `catUnidad`, `EmpresaDatosBancarios`, `tpPartidaPedido` y el `TimbreFiscalDigital` del XML retornado por el PAC (TurboPac).
...
- La comunicación con ProquifaDotNet (.NET Framework 4.8) para obtener los datos del pedido se realiza mediante llamadas entre APIs. Los datos del CFDI (`CFDIGenerada`) se consultan directamente, ya que la tabla es propiedad de ProquifaDotNet.Finanzas.
- **Relación con RE-FU-019:** La Tarea 15 de RE-FU-019 (`AdvanceInvoicePreviewService`) usa este mismo mapeo con un subconjunto de campos (sin UUID, sin sello, sin cadena original) para el PDF de previsualización. Este servicio provee el modelo completo para ambos contextos (preview y definitivo); no duplica RE-FU-019, lo complementa con los campos SAT pendientes.

**Objetivo general:**
Implementar el servicio que consolida todos los datos necesarios del CFDI 4.0 en un modelo unificado (`InvoicePdfModel` o equivalente), listo para ser consumido por el generador de PDF de DocumentBuilder, incluyendo branding por empresa emisora, datos fiscales del emisor y receptor, partidas con claves SAT, totales e impuestos, elementos técnicos SAT y QR.
...
- `R16A-RE-FU-021.md` — Criterios A1–J2
- R16A-RE-FU-019 Tarea 15 — `AdvanceInvoicePreviewService` (placeholder de template)
...
- La persistencia utiliza el patrón existente: `INSERT Archivo` con `FileBucket='facturas'` y la referencia se almacena en `CFDIGenerada.IdArchivoPdf`.
...
- La actualización de `CFDIGenerada.IdArchivoPdf` se realiza directamente en BD (vía EF Core), ya que `CFDIGenerada` es propiedad de ProquifaDotNet.Finanzas; no requiere llamada API a ProquifaDotNet.
- **Relación con RE-FU-019:** La Tarea 13 de RE-FU-019 (`AdvanceInvoiceGenerateService`) reservó los pasos 10–11 del flujo (almacenar PDF+XML en Minio, INSERT Archivo x2) con un placeholder indicando que el PDF real se define en requisito independiente. Esta tarea implementa esos pasos reales con el PDF completo CFDI 4.0; no es una tarea nueva duplicada, es la implementación definitiva de lo que RE-FU-019 dejó pendiente.
...
**[ RE-FU-021 ] [IMP-EXIST-SERVICE] Extender AdvanceInvoiceGenerateService con persistencia real del PDF tras timbrado (pasos 10-11 RE-FU-019 T13)**
...
**Resultado esperado:**
`AdvanceInvoiceGenerateService` con los pasos 10-11 implementados definitivamente: tras el timbrado exitoso del PAC, el PDF CFDI 4.0 se genera y persiste en Minio como artefacto fiscal inmutable antes de retornar el response al cliente.

**Entregables:**
- `AdvanceInvoiceGenerateService` actualizado (pasos 10-11 reales, sin placeholder)
- Prueba de integración: flujo completo generar FAA México → PDF persistido en Minio

**Criterios de aceptación:**
- Tras el timbrado exitoso, el PDF se persiste en Minio y la referencia queda en `CFDIGenerada.IdArchivoPdf` antes de retornar el response.
...
**Más información de la tarea:**
Ver sección "Integración con RE-FU-019 — T13 pasos 10-11" en `R16A-RE-FU-021-Back.md`. Ver Tarea 13 en `R16A-RE-FU-019-Tareas.md` (placeholder original).

**Recursos:**
- `R16A-RE-FU-021-Back.md` — Sección Parte B, integración T13 pasos 10-11
- R16A-RE-FU-019 Tarea 13 — `AdvanceInvoiceGenerateService` (placeholder de pasos 10-11)

---

## TAREA 12

**[ RE-FU-021 ] [IMP-EXIST-SERVICE] Extender AdvanceInvoicePreviewService con template real de Factura México (RE-FU-019 T15)**
...
**Resultado esperado:**
`AdvanceInvoicePreviewService` con la generación real del PDF de previsualización usando los templates CFDI 4.0 de DocumentBuilder, diferenciado por empresa emisora, con todos los campos disponibles excepto los del `TimbreFiscalDigital`.

**Entregables:**
- `AdvanceInvoicePreviewService` actualizado (template real, sin placeholder)
...
- El flujo existente de `AdvanceInvoicePreviewService` (pasos previos al template) no presenta regresiones.
- Las 4 empresas emisoras (GOL/MUN/PRO/PQF) generan correctamente su PDF de preview con el template correspondiente.

**Más información de la tarea:**
Ver sección "Integración con RE-FU-019 — T15 template" en `R16A-RE-FU-021-Back.md`. Ver Tarea 15 en `R16A-RE-FU-019-Tareas.md` (placeholder original).

**Recursos:**
- `R16A-RE-FU-021-Back.md` — Sección Parte B, integración T15 template preview
- R16A-RE-FU-019 Tarea 15 — `AdvanceInvoicePreviewService` (placeholder de template)
...
**Criterios de aceptación:**
- Tras el timbrado exitoso en el flujo de Validar Cobro, el PDF se persiste en Minio y la referencia queda en `CFDIGenerada.IdArchivoPdf` antes de transicionar al estado "Pendiente en Validar Pago".
