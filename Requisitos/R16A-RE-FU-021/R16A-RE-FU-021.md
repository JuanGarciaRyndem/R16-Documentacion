...
- ~~Pedidos para clientes con Región Perú: la Factura Perú se documenta en requisito independiente.~~ **[2026-08-21 · DUDA-052]** Obsoleto: no se trata de "documentar aparte", se CANCELA POR COMPLETO la facturación de Perú en R16 (no solo se restringe a Prepago). No hay ningún desarrollo de facturación para Perú en este release; el requisito independiente de Factura Perú (R16A-RE-FU-022) no se ejecuta.
...
Entonces ~~pendiente confirmar si la construcción de la referencia sigue el mismo método que en la Proforma o si tiene reglas propias para la Factura.~~ **[2026-08-21 · DUDA-059 · Resuelta]** deberá construirse con el MISMO método utilizado en la Proforma (lógica Banamex/no-Banamex + Código Validador); no aplican reglas propias para la Factura.
...
** [2026-08-21 · DUDA-061 · Descartada] La duda formal sobre el diseño completo de la Factura Anticipo se DESCARTÓ (no se resolvió con un diseño definitivo): se determinó que sustancias controladas NO está contemplado dentro del alcance de Región Perú, y que la pregunta se resuelve por la vía de DUDA-045 (fuera de este batch). Esto NO define el diseño de la Factura Anticipo para México: el diseño detallado sigue siendo orientativo/sugerido y pendiente de confirmar con el asesor comercial, tal como ya indica esta sección. No se debe tomar nada de lo siguiente como especificación cerrada. **
...
- ~~Pendiente confirmar si la construcción de la referencia bancaria del cliente en la Factura sigue el mismo método que en la Proforma (lógica Banamex con concatenación de segmentos / lógica no-Banamex con nombre del cliente directo) o si tiene reglas propias.~~ **[2026-08-21 · DUDA-059 · Resuelta]** Se decide internamente utilizar la MISMA referencia bancaria/método que la Proforma; no hay reglas propias para la Factura.
- ~~Pendiente confirmar la diferencia entre el número de orden de compra del cliente (referencia externa proveniente del Pedido) y el Folio del Pedido Interno (PI) del sistema PQF2 que aparece en la sección de identificadores. Aclarar si son datos distintos o complementarios.~~ **[2026-08-21 · DUDA-060 · Resuelta]** Son datos DISTINTOS y no intercambiables: la "Referencia" es la capturada en Buzones (orden de compra del cliente); el "Folio de PI" es el generado en la Confirmación de Pedido por PQF2.
...
- ~~Pendiente decisión técnica: tipo de almacenamiento del PDF persistido en base de datos (archivo binario completo vs snapshot estructurado de datos). Decisión transversal con Proforma.~~ **[2026-08-21 · DUDA-039 · Resuelta]** Los PDF SE ALMACENAN EN EL SISTEMA como archivo (no se reconstruyen desde datos) y NO sufren regeneración: se conserva el PDF generado originalmente. Consistente con la implementación descrita en R16A-RE-FU-021_BD.md (persistencia vía Minio/tabla Archivo, referenciada desde CFDIGenerada.IdArchivoPdf).
- ~~Pendiente — maquetas de Golocaer S.A.C. Perú no disponibles; criterios de detalle de modales se validarán contra ellas cuando lleguen.~~ **[2026-08-21 · DUDA-052]** Obsoleto: se cancela por completo la facturación de Perú en R16; ya no aplica validar maquetas de Golocaer S.A.C. Perú para este alcance.
- ~~Duda — Una factura electrónica real de Golocaer S.A.C. (E001-362, 08/05/2026) recibida como muestra fue emitida a Crédito (con cuotas y fecha de vencimiento), timbrada ante SUNAT. Esto contrasta con el requisito del cliente de que en Perú el flujo R16 aplica solo a Prepago. Pendiente confirmar: (1) si Golocaer factura habitualmente a crédito en su operación corriente (ajena a R16) o si el caso prepago es el único contemplado; (2) confirmar que el alcance de R16 para Perú se mantiene exclusivamente en Prepago. Mientras tanto, se conserva la decisión vigente: Perú = solo Prepago.~~ **[2026-08-21 · DUDA-052 · Descartada]** La duda se descarta porque cambia la premisa de fondo: SE CANCELA LA FACTURACIÓN DE PERÚ POR COMPLETO en R16 (no solo se restringe a Prepago). Ya no hay ningún desarrollo de facturación para Perú en este release, por lo que el caso de la factura a Crédito de Golocaer deja de ser relevante para R16.
