# Impacto en Back — TPSC-RE-FU-028
...
| Envío             | ProquifaDotNet.Finanzas   | Modal envío, integración Brevo (correo con PDF + XML adjuntos)                          |
...
| `tpProformaAdelanto.IdCFDIGenerada`                 | RE-FU-015/019 | UUID de la FAA para `CFDIRelacionados` del Complemento de Pago                                                                                         |
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — Catálogo Nuevo: `catTipoDocumentoFiscal`.
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — Catálogo Nuevo: `catDocumentoFiscalCobroEstado`.
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — Catálogo Nuevo: `catTipoCFDI`.
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — Tabla Nueva: `fccDocumentoFiscalCobro`.
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — Tabla Nueva: `fccConfirmacionPedido`.
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — ALTER TABLE CFDIGenerada.
...
> Ver script completo en `TPSC-RE-FU-028_BD.md` — CREATE VIEW vfccDocumentoFiscalCobro.
...
2. Para líneas `FACTURA` y `FACTURA_ANTICIPO`: invoca `FacturaMexicoPdfMappingService.MapearPreviewAsync(idCFDIGenerada)` (RE-FU-021) — consolida datos fiscales en `FacturaPdfModel` sin `TimbreFiscalDigital`, resuelve `TemplateKey` dinámicamente (`GOL/MUN/PRO/PQF_MEX_FAC`) y genera PDF en memoria vía DocumentBuilder.
3. Para líneas `COMPLEMENTO_PAGO`: la previsualización del PDF del Complemento se implementa en **TPSC-RE-FU-030** (Diseño y generación: Complemento de Pago México).
...
4. Finanzas: invoca `PersistirFacturaMexicoPdfService.PersistirAsync(IdCFDI, xmlTimbrado)` (RE-FU-021 GAP-10) — genera PDF definitivo con `TimbreFiscalDigital`, lo sube a MinIO, INSERT `Archivo`, UPDATE `CFDI.IdArchivoPdf`.
...
3. Finanzas: invoca `PersistirFacturaMexicoPdfService.PersistirAsync(IdCFDIFactura, xmlFactura)` (RE-FU-021 GAP-10).
...
6. Finanzas: la persistencia del PDF del Complemento de Pago (plantilla `*_MEX_COP`, MinIO) se implementa en **TPSC-RE-FU-030** — en este requisito se depende de ese servicio.
...
1. Finanzas → Timbrado: solicita timbrado de Complemento de Pago referenciando el UUID de la FAA existente (`tpProformaAdelanto.IdCFDIGenerada`).
...
### B6 — Modal de Envío y despacho con Brevo

**Descripción:** Al presionar "Enviar" en una línea en estado `GENERADO`, Finanzas abre el modal de envío y, al confirmar, despacha el correo vía Brevo con los adjuntos correspondientes.
...
|---|---------------|------|-------------|-----------------|------------|
| E1 | ETL Datos Buzón de Cobros | Datos | TPSC-RE-FU-008 | **RE-FU-028** | Al confirmar cobro en Paso 1 |
| E2 | ETL Datos Proforma | Datos | TPSC-RE-FU-016 | **RE-FU-028** | Al tramitar pedido Prepago |
| E3 | ETL Datos Factura | Datos | TPSC-RE-FU-019 | **RE-FU-028** | Al enviar línea Paso 3 (Factura o Factura Anticipo) |
| E4 | ETL Datos Complemento de Pago | Datos | TPSC-RE-FU-030 | **RE-FU-030** | Al enviar línea Paso 3 (Complemento) |
| E5 | ETL Datos Nota de Crédito | Datos | TPSC-RE-FU-032 | **RE-FU-032** | Al enviar línea Paso 3 (si hay NCs aplicadas) |
| E6 | Transferencia PDF Factura | Documento | TPSC-RE-FU-021 | **RE-FU-028** | Al enviar línea Paso 3 (Factura o Factura Anticipo) |
| E7 | Transferencia PDF Complemento de Pago | Documento | TPSC-RE-FU-030 | **RE-FU-030** | Al enviar línea Paso 3 (Complemento) |
| E8 | Transferencia PDF Nota de Crédito | Documento | TPSC-RE-FU-034 | **RE-FU-034** | Al enviar línea Paso 3 (si hay NCs aplicadas) |

> **Alcance de RE-FU-028:** Solo se implementan E1, E2, E3 y E6. Los ítems E4 y E7 (Complemento de Pago) se implementan en TPSC-RE-FU-030. Los ítems E5 y E8 (Nota de Crédito) se implementan en TPSC-RE-FU-032 y TPSC-RE-FU-034 respectivamente.
...
**Dependencia:** TPSC-RE-FU-008 — Buzón de Cobros (define el modelo del cobro y sus datos).
...
**Dependencia:** TPSC-RE-FU-016 — Proforma México (define estructura y datos de la proforma).
...
**Dependencia:** TPSC-RE-FU-019 — Factura por Adelantado Detalle México (define el modelo de `CFDIGenerada` y el flujo de timbrado).
...
> Este ítem **no se implementa en RE-FU-028**. El diseño, estructura fiscal y ETL del Complemento de Pago se definen y se implementan en **TPSC-RE-FU-030 — Diseño y generación de Documentos: Complemento de Pago México**.

---

### E5 — ETL Datos Nota de Crédito a Legacy *(fuera de alcance RE-FU-028)*

> Este ítem **no se implementa en RE-FU-028**. El modelo de la NC, su estructura fiscal y su ETL a Legacy se definen en **TPSC-RE-FU-032 — Notas de Crédito: México**.
...
**Dependencia:** TPSC-RE-FU-021 — Diseño y generación PDF Factura México (define el PDF generado por `PersistirFacturaMexicoPdfService` almacenado en MinIO).
...
> Este ítem **no se implementa en RE-FU-028**. El PDF del Complemento de Pago (plantilla `*_MEX_COP`) y su transferencia a Legacy se definen en **TPSC-RE-FU-030 — Diseño y generación de Documentos: Complemento de Pago México**.

---

### E8 — Transferencia PDF Nota de Crédito a Legacy *(fuera de alcance RE-FU-028)*

> Este ítem **no se implementa en RE-FU-028**. El PDF de la Nota de Crédito y su transferencia a Legacy se definen en **TPSC-RE-FU-034 — Diseño y generación de Documentos: Nota de Crédito México**.
...
| ---------------------------------- | ------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------- |
| PDF Factura México                 | `GOL/MUN/PRO/PQF_MEX_FAC` | **Existente** (RE-FU-021)                                     | `FacturaMexicoPdfMappingService` + `PersistirFacturaMexicoPdfService` |
| PDF Factura Anticipo México        | `GOL/MUN/PRO/PQF_MEX_FAC` | **Existente** (RE-FU-021, misma plantilla con datos anticipo) | `FacturaMexicoPdfMappingService` + `PersistirFacturaMexicoPdfService` |
| PDF Complemento de Pago México     | `GOL/MUN/PRO/PQF_MEX_COP` | **Nueva — definir en RE-FU-030**                              | Ver TPSC-RE-FU-030 — Diseño y generación: Complemento de Pago México  |
...
> La plantilla `*_MEX_COP` (Complemento de Pago) se define en TPSC-RE-FU-030.
...
> El canal de transferencia (tabla ETL, cola RabbitMQ, llamada API Legacy directa) no está definido para las transferencias implementadas en RE-028: E1 (Buzón de Cobros), E2 (Proforma), E3 (Factura) y E6 (PDF Factura). Es prerequisito para implementar las acciones post-envío. Pendiente definir con arquitectura antes de implementar. Ver Parte E — Consideraciones transversales ETL. Las transferencias de Complemento de Pago (E4, E7) y Notas de Crédito (E5, E8) están sujetas a la misma brecha pero se resuelven en TPSC-RE-FU-030 y TPSC-RE-FU-032/034 respectivamente.
