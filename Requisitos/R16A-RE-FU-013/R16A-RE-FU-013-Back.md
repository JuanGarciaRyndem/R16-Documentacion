...
| GAP-04 | Previsualizacion obligatoria del PDF | Endpoint que retorna el PDF generado para previsualizacion antes del envio. El ESAC acepta y luego se ejecuta envio en segundo llamado. **Debe soportar cancelacion:** si el ESAC cancela, se vuelve al pedido sin enviar la proforma (Criterio C2). Vinculado con GAP-01 (política de folio) y con BD Gap 4 (IdArchivoPDF en tpProformaPedido — verificar FK al archivo para recuperar el PDF). | Medio |
...
- Confirmar con el cliente la política del folio de proforma al cancelar la previsualización
