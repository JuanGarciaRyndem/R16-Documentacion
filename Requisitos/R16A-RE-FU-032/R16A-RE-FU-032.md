...
El sistema debe contar con un módulo independiente de Notas de Crédito para Región México, aplicable exclusivamente a clientes Prepago en este release y operado por el área de Tesorería. El módulo expone una pantalla de consulta de NCs agrupada por cliente y un wizard de generación que permite seleccionar la factura origen, capturar el motivo y los datos de la NC (en modalidad por partidas o manual), confirmar con previsualización del PDF y emitir el documento timbrado.

El módulo se acopla a Validar Cobro: las NCs generadas quedan disponibles para su aplicación durante la asociación de cobros. Las NCs históricas previas al go-live no se importan desde Legacy. La estructura funcional del módulo se reutiliza para Perú con diferencias significativas y se documenta en requisito independiente.
...
- Políticas de autorización por monto (umbrales que requieran autorización de coordinador o director): **PMO #54**.
...
**Dado que** el motivo es Devolución de mercancía,
**Cuando** el sistema renderiza el Paso 2,
...
**Dado que** la NC es parcial o la factura origen está en mes anterior al actual,
**Cuando** el sistema renderiza el Paso 2,
...
**Dado que** la NC es de R16 prepago,
...
**Dado que** la moneda es extranjera,
...
**Dado que** el timbrado y el envío fueron exitosos,
...
**Dado que** el timbrado fue exitoso (o se consulta una NC desde el listado),
...
**Dado que** existe la vista de NC Emitida,
**Cuando** el sistema renderiza,
...
**Dado que** existe el modal de envío del correo de la NC,
...
**Dado que** existen cálculos del sistema,
...
**Dado que** existe el renderizado de importes,
...
- Fila documenta el módulo NC completo para Región México. Perú en requisito independiente.
...
- El diseño del PDF representativo de la NC se documenta en requisito independiente (análogo a R16A-RE-FU-021 Factura México). Esta fila cubre solo funciones e información del módulo.
