# R16A-RE-FU-020 — Factura por Adelantado: Detalle Peru
...
| **Historia de Usuario** | Yo como **Gestor de Cobranza**, quiero contar con la pantalla de Detalle por cliente en Factura por Adelantado que me permita generar, timbrar ante SUNAT y enviar la factura electrónica de cada pedido Prepago pendiente de clientes con Región Perú, para emitir oportunamente las facturas por adelantado de Golocaer S.A.C. y dar continuidad al flujo de cobro en Validar Cobro, cumpliendo la normativa SUNAT. |
| **Prioridad**           | Alta                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Estado**              | Propuesto                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- Pedidos Crédito de la región Perú: la Factura por Adelantado no se ofrece para Crédito en Perú, porque el timbrado fiscal peruano en R16 se limita a Prepago. Decisión de alcance del proyecto (ver R16A-RE-FU-012).
...
- Clientes con Región México (cubiertos en R16A-RE-FU-019).
...
- Generación del PDF de la factura como artefacto (estructura visual): se documenta en R16A-RE-FU-022.
...
El catálogo de productos actual no contiene el código SUNAT del producto, la unidad de medida SUNAT ni la afectación al IGV por línea, datos obligatorios en el XML UBL 2.1. Sin ellos no es posible timbrar la factura, por lo que esta brecha puede detener el desarrollo del flujo de facturación Perú si no se resuelve antes (ver Brecha 1 de R16A-RE-FU-005).

Riesgo 2 — Brecha de timbrado SUNAT/OSE no resuelta
La habilitación para Región Perú depende de la integración con un OSE/PSE autorizado por SUNAT, brecha mayor no resuelta del proyecto (ver Brecha 5 de R16A-RE-FU-005). Mientras no se resuelva, los clientes Perú no pueden completar el timbrado.
...
Riesgo 4 — Solapamiento de denominación de rol (Resuelto DUDA-047, 2026-08-21)
~~La denominación del rol que opera este módulo aparece como "Gestor de Cobranza" en la matriz del cliente y como "Analista de Cuentas por Cobrar" en las sesiones de revisión de pantallas, sin una denominación canónica resuelta.~~ Resuelto: la denominación canónica del rol en la documentación funcional es "Gestor de Cobranza".
...
Entonces deberá consignar el Tipo de Operación del catálogo 51 SUNAT. ** Pendiente definir para la maqueta si el operador lo selecciona (combo editable, como el Uso CFDI de México) o si el sistema lo fija siempre en "0101 - Venta interna". Si es seleccionable, este sería el único campo configurable del modal; si es fijo, el modal es de solo lectura. Ver Regla 7 y Brecha 3 de R16A-RE-FU-005. **
...
- Fila para el detalle/generación de la Factura por Adelantado de la Región Perú (solo Prepago), contraparte de R16A-RE-FU-019 (México). Estado depende de la resolución de las brechas Perú.
...
- Decisión de alcance: la Factura por Adelantado NO aplica a Crédito en Perú porque el timbrado peruano en R16 se limita a Prepago (ver R16A-RE-FU-012). De Perú nada se transfiere a Legacy.
...
- Dato configurable por factura: el equivalente al Uso CFDI mexicano es el Tipo de Operación SUNAT (catálogo 51). ** Pendiente definir si es editable por el operador o lo fija el sistema (ver Brecha 3 de R16A-RE-FU-005). **
- ** Brecha bloqueante — Datos fiscales SUNAT del producto (código SUNAT, unidad de medida SUNAT, afectación al IGV por línea) inexistentes en el catálogo de productos actual; obligatorios para emitir el CPE. Puede detener el desarrollo del flujo de facturación Perú si no se resuelve antes (ver Brecha 1 de R16A-RE-FU-005). **
...
- El RUC del cliente se valida en el Catálogo de Clientes (R16A-RE-FU-004); este módulo lo consume ya validado, no lo revalida.
- ** Brecha — Integración SUNAT/OSE-PSE para timbrado del CPE. El proveedor de timbrado para Perú está pendiente de definir (no se reutiliza el PAC de México). Brecha mayor documentada en Brecha 5 de R16A-RE-FU-005. **
...
- **(Resuelto DUDA-047, 2026-08-21):** la denominación canónica del rol operativo en la documentación funcional es "Gestor de Cobranza"; se eliminan las menciones a "Analista de Cuentas por Cobrar" como nombre del rol. Nota: este requisito se encuentra en la carpeta "Cancelado"; se homologa igualmente por consistencia documental.
