...
- **Enriquecimiento progresivo:** cuando el pedido pasa de una etapa a otra (`pcPromesaDeCompra` → `ppPedido` → `tpPedido`), el mismo registro se localiza por la FK que ya tiene poblada y se actualiza para agregar la FK de la nueva etapa — no se crea un registro nuevo. T8 ajusta, dentro de `ProquifaDotNet`, los puntos de alta y enriquecimiento de las etapas Buzón → Pretramitar → Intramitable/OC Ajustada → Tramitar → cadena logística (L03–L09) para que inserten/actualicen este registro en cada transición (ver tabla de puntos de integración en `-Tareas.md`, T8) — no solo el cierre de Tramitar Pedido con FAA propio de este requisito. La cancelación queda fuera de alcance en todos los casos (ver más abajo).
