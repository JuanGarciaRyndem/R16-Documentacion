...
- Entrega con Remisión, que no aplica para clientes prepago en ninguna variante.
...
**Regla 1 — Factura por Adelantado disponible para Prepago sin controlados**
Para pedidos de clientes Prepago sin sustancias controladas, el sistema ofrece la opción de activar Factura por Adelantado; el ESAC decide si la activa o no.

**Regla 2 — Entrega con Remisión no aplica para Prepago**
Para pedidos de clientes Prepago (con o sin sustancias controladas), la opción de Entrega con Remisión no aplica en ninguna variante.
...
Una vez generado el pendiente en Factura por Adelantado, el sistema cierra el pendiente operativo del pedido en la bandeja de Tramitar Pedido, de modo que el pedido ya no aparece como acción pendiente para el ESAC. Este cierre se refiere únicamente al pendiente operativo de esa bandeja: la acción a realizar en Tramitar Pedido finalizó y el pedido avanza al siguiente módulo (Factura por Adelantado y, después, Validar Cobro). El estatus real del pedido se consulta fuera de las bandejas de pendientes.
...
- **Entonces** el sistema no deberá permitir editar los datos de facturación del cliente; estos deberán presentarse en modo solo lectura, tomados del catálogo del cliente vigente al momento de la activación.
...
- **Entonces** el pedido no deberá seguir apareciendo como pendiente en la bandeja del módulo Tramitar Pedido del ESAC, entendiéndose que el pendiente operativo de esa bandeja terminó y el pedido avanzó al siguiente módulo, no que el pedido quedó tramitado en su totalidad (la Confirmación de Pedido de prepago se genera al validar el cobro). La trazabilidad del estatus del pedido se consulta fuera de las bandejas de pendientes.
...
- Cubre tres requisitos del cliente: tramitación de pedidos prepago en México y Perú; activación de Factura por Adelantado para prepago; y cadena de pendientes generada al tramitar.
...
- La opción de Entrega con Remisión no aplica para clientes prepago en ninguna variante.
...
| 3 | 2026-09-11 | Correcciones de consistencia (cierre) | Se completan las correcciones de consistencia pendientes de la pasada anterior: se retira el lenguaje de control de pantalla ("botón Editar Datos", "radio button", "se muestra la pantalla") de los Criterios A3 y A4 y del título de la Sección A, redactándolos en términos de comportamiento del sistema. Se elimina el Criterio D2 (Momento de generación del pendiente Validar Cobro), no verificable desde esta fila de la matriz; su contenido se conserva como contexto únicamente en la Regla 6. Se renumeran los criterios restantes de la Sección D de forma consecutiva (D3→D2, D4→D3, D5→D4), conservando la nota de resolución OBS-027 inmediatamente después del criterio D4. Se retiran las referencias cruzadas "(ver Criterio D5)" de la Regla 7 y del antiguo Criterio D3. Se corrigen las Notas: se quita "con emisión de Confirmación" (la Confirmación de Pedido de prepago se genera al validar el cobro, no al tramitar) y se reescribe la nota sobre Entrega con Remisión sin lenguaje de control de pantalla. |
