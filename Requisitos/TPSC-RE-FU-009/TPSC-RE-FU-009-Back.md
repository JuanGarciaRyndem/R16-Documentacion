# TPSC-RE-FU-009 — Impacto de Backend
...
| # | Hallazgo | Impacto |
|:-:|----------|---------|
| 1 | `ArchivoCliente` sin registros activos | La funcionalidad de carga de documentos (TPSC-RE-FU-003) es **prerequisito** — sin ella, la validación siempre bloquearía |
...
|-----------|----------|-------------------|
| **TPSC-RE-FU-003** | **Prerequisito** | Carga de documentos regulatorios en `ArchivoCliente`. Sin esto, la validación siempre bloquea porque no hay documentos registrados. |
| **TPSC-RE-FU-007** | **Compartido** | Misma actualización de `fnEsProductoControlado` para agregar `origen`. El script de BD debe ejecutarse una sola vez. |
| **TPSC-RE-FU-005** | **Referencia** | La `Region` del cliente determina qué documentos se requieren. |
