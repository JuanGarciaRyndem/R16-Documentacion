# Impacto en Back — TPSC-RE-FU-022
...
> Los campos `CodigoSUNAT` (Producto), `ClaveSUNAT` (catUnidad) e `IdCatAfectacionIGV` (Producto) son **obligatorios** en el XML UBL 2.1 y en el PDF del CPE. No existen en el catálogo de productos actual. Documentado en TPSC-RE-FU-005 Brecha 1. Scripts propuestos en RE-FU-020.

> ⛔ **BRECHA BLOQUEANTE — Integración OSE/PSE SUNAT (B2)**
> El PDF se genera al timbrarse exitosamente ante SUNAT/OSE. Mientras la integración OSE/PSE no esté resuelta (RE-FU-020 brecha mayor), no hay timbrado y por tanto no hay PDF. Documentado en TPSC-RE-FU-005 Brecha 5.
...
```sql
-- BLOQUEANTE para partidas del PDF Perú — ver scripts en TPSC-RE-FU-020-Back.md Parte C
