# Contexto + Checklist + Workflow para revisión de PRs — Timbrado (R16)
...
Repo local a revisar para el proyecto de Timbrado (R16 - Timbrar Factura con Turbo PAC):

```
C:\Users\juan.garcia\Documents\proquifa-pqf2-timbrado-api
```

Este repositorio corresponde a la API de Timbrado
(proquifa-pqf2-timbrado-api) y será la referencia base para el
análisis/revisión del proyecto de Timbrado.

- Organización/owner en GitHub: `ryndem`
- Nombre exacto del repo en GitHub: `ryndem/proquifa-pqf2-timbrado-api`
...
```bash
gh api repos/ryndem/proquifa-pqf2-timbrado-api/pulls/{N} --jq "{base: .base.ref}"
```
...
`ryndem/proquifa-pqf2-timbrado-api` (proyecto R16 - Timbrar Factura con Turbo PAC /
Timbrado).
...
```bash
gh api repos/ryndem/proquifa-pqf2-timbrado-api/pulls/{N} --jq "{base: .base.ref, base_sha: .base.sha, head_sha: .head.sha}"
```
...
```bash
"C:\Program Files\GitHub CLI\gh.exe" api repos/ryndem/proquifa-pqf2-timbrado-api/pulls/{N}/reviews -X POST --input review.json
```
...
```bash
"C:\Program Files\GitHub CLI\gh.exe" api repos/ryndem/proquifa-pqf2-timbrado-api/pulls/{N}/reviews --jq ".[] | {id, state}"
"C:\Program Files\GitHub CLI\gh.exe" api repos/ryndem/proquifa-pqf2-timbrado-api/pulls/{N}/reviews/{REVIEW_ID} -X DELETE
```
