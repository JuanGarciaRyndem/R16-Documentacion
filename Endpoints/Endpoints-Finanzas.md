...
| PUT    | `/api/v1/validate-collection/payment`                     | INSERT borrador (`Confirmado=0`) / UPDATE borrador del cobro (Paso 1). El body incluye ambos TC: `TipoDeCambio` vs MXN (fiscal) y `TipoDeCambioMonedaFacturacion` vs moneda de facturación del cliente (operativo — OBS-050). Ambos pre-cargados con DOF de `FechaPago` (OBS-049) y editables. | Body: `ClientPaymentDto`                                                       | `ClientPaymentDto` con Id asignado              |
| GET    | `/api/v1/validate-collection/proforma`                    | Lista pedidos pendientes del cliente para modal Gestionar Cobranza (`MontoPendiente > 0 AND IdcatEstadoProforma <> Cancelada`)                                     | `?clientId={guid}`                                                             | `List<ProformaDto>` + header `MontoTotalPendiente` |
| PUT    | `/api/v1/validate-collection/proforma/estimated-payment-date` | Actualiza fecha estimada de pago; genera historial en `fccFechaEstimadaPagoHistorial` (OBS-044)                                                                | Body: `List<{ IdTpProformaPedido, Fecha, IdUsuarioCambio, Motivo? }>`          | `204 NoContent`                                 |
...
| PUT    | `/api/v1/validate-collection/orders/{orderId}/cancel-non-payment` | **Orquestador de cancelación distribuida** (Caso A sin CFDI / Caso B con CFDI, Conducta 2). Ejecuta la secuencia: (1) si hay CFDI timbrado → llama Timbrado `POST /api/v1/invoices/{invoiceId}/cancel` con `claveMotivo=03`; (2) llama ProquifaDotNet `PUT /api/v1/orders/{orderId}/cancel-non-payment` para trazabilidad en `tpPedido` (idempotente); (3) `UPDATE tpProformaPedido.IdcatEstadoProforma = Cancelada` (commit local — paso decisivo que saca el pedido del listado). Programa job de Hangfire para polling del SAT si el CFDI queda `EnProceso`. Envía correo a soporte si el SAT rechaza (sin reapertura automática). No es transacción atómica única — consistencia por orquestación + idempotencia | `orderId` en path | `202 Accepted` (Caso B, CFDI EnProceso) / `204 NoContent` (Caso A o CFDI Cancelado síncrono) / `409 Conflict` (SAT rechaza síncrono / `tpPedido` no cancelable) |
...
| PUT | `/api/v1/validate-collection/payment/{id}/edit` | Edita un cobro ya capturado (`Confirmado=1`) mientras `BloqueadoPorTimbrado=0`; re-pre-carga ambos TC (`TipoDeCambio` vs MXN y `TipoDeCambioMonedaFacturacion` vs moneda de facturación — OBS-050) si cambia moneda/fecha y el usuario no los editó manualmente | `id` en path; Body: campos editables del cobro (incluye ambos TC) | `ClientPaymentDto` actualizado |
...
| POST   | `/api/v1/validate-collection/client/{idCliente}/balance/calculate`       | Calcula el saldo dinámico de la asociación (multi-divisa) al seleccionar/deseleccionar elementos. **OBS-052:** la conversión cobro→documento usa el **TC del pago** (`fccPagoCliente.TipoDeCambioMonedaFacturacion`), NO el TC de emisión del documento. Diferencial contra TC de emisión = fluctuación cambiaria (LISR / NIF B-15), no saldo del cliente. | `idCliente` en path; Body: `CalculateBalanceRequestDto`    | `CalculateBalanceResponseDto` |
