...
> **[REVISIÓN — Juan David García Cruz — 2026-07-30]**
>
> El documento está bien estructurado y cubre en detalle los tres flujos principales (captura, consumo y consulta). Las reglas técnicas RT-B01 a RT-B19 son coherentes con las decisiones de arquitectura ya tomadas en el ecosistema.
> A continuación se detallan las observaciones puntuales identificadas. Las de mayor impacto en diseño están marcadas con 🔴; las menores con 🟡.
...
[**Diseño funcional detallado	11**](#diseño-funcional-detallado)
...
> 🟡 **[OBS-R-002] Inconsistencia en la notación del routing key:** El diagrama Mermaid usa `{app}.{entidad}.{operacion}` (abreviado), mientras que la tabla de componentes y el paso 4 del Flujo 1 usan `{aplicativo}.{entidad}.{operacion}`. Homogeneizar a la forma larga `{aplicativo}.{entidad}.{operacion}` en el diagrama para evitar ambigüedad.
...
> 🔴 **[OBS-R-003] TTL escalonado (2s/10s/30s) con una sola cola Retry — implementación ambigua:**
> `x-message-ttl` es un atributo de cola, no de mensaje individual. Con una sola cola `Bitacoras.Eventos.Retry`, todos los mensajes expirarían con el mismo TTL fijo — no es posible aplicar 2s en el primer reintento, 10s en el segundo y 30s en el tercero usando esa propiedad.
>
> **Alternativas para implementar el backoff escalonado real:**
> 1. **Tres colas separadas** — `Bitacoras.Eventos.Retry.2s` (TTL 2 000 ms), `Bitacoras.Eventos.Retry.10s` (TTL 10 000 ms), `Bitacoras.Eventos.Retry.30s` (TTL 30 000 ms), cada una con DLX hacia `Bitacoras.Eventos`. El worker lleva el contador de intentos en un header AMQP y elige la cola de destino según el intento actual.
> 2. **Per-message TTL** (`x-expiration` header) con una sola cola — RabbitMQ sí permite TTL por mensaje mediante este header, pero solo funciona con colas clásicas y el comportamiento de expiración no es garantizado cuando hay mensajes con TTL mayor delante en la cola. No recomendado para este patrón.
>
> **Recomendación:** optar por la opción 1 (tres colas). Actualizar la sección de infraestructura (Impacto §2) y el diagrama Mermaid del Flujo 2 para reflejar esta topología. El diagrama actual ya muestra el flujo correcto en lógica — solo falta precisar que son tres colas distintas en la topología real.
...
> 🟡 **[OBS-R-007] RT-B14 — ¿`Superusuario` también debe tener acceso?**
> La regla restringe el endpoint exclusivamente al rol `Auditor`. Aclarar si `Superusuario` (rol ya existente en IdentityServer) debe poder consultar bitácoras también, o si el acceso debe ser exclusivo del rol nuevo. Si `Superusuario` queda excluido, documentar explícitamente la razón (separación de responsabilidades, principio de mínimo privilegio). Si debe incluirse, agregar `[Authorize(Roles = "Auditor,Superusuario")]` y actualizar la regla.
...
> 🔴 **[OBS-R-004] Índice compuesto faltante — Application + Entity + EntityId:**
> El índice compuesto actual (`Entity, EntityId, Timestamp desc`) no incluye `Application`. En un ecosistema con 5 aplicativos, el nombre lógico de una entidad puede colisionar entre aplicativos: tanto ProquifaDotNet como ProquifaDotNet.Finanzas podrían auditar una entidad llamada `"Cliente"`, y el índice actual mezclaría ambas en la misma consulta.
>
> **Recomendación:** cambiar el índice compuesto principal a `(Application, Entity, EntityId, Timestamp desc)`. Este índice sería el camino más eficiente para la consulta más frecuente ("historial de la entidad X del aplicativo Y") y también satisface las consultas solo por `Application`. El índice `Simple` en `Application` quedaría redundante y podría eliminarse.
...
> 🟡 **[OBS-R-005] Filtro por `Operation` ausente en `BitacoraQueryRequest`:**
> Un caso de uso típico de auditoría es "mostrar todos los registros eliminados de la entidad X". El contrato actual no permite filtrar por `Operation` (Create/Update/Delete). Como ya existe como `BitacoraOperationEnum` en el contrato del evento, agregar `Operation: BitacoraOperationEnum?` al request sería bajo costo y alto valor para auditores.

> 🔴 **[OBS-R-006] `TotalCount` en colecciones *append-only* de alto volumen — riesgo de rendimiento:**
> `countDocuments()` en MongoDB hace un scan del índice completo para calcular el total exacto. En una colección que crece indefinidamente, esta operación puede volverse costosa bajo carga.
>
> **Alternativas a evaluar:**
> 1. **Cursor-based pagination** (keyset) — eliminar `TotalCount`/`TotalPages` y devolver un cursor para la siguiente página. Es la alternativa más escalable para logs de auditoría.
> 2. **`estimatedDocumentCount()`** para el total general (muy rápido, sin filtros) y `countDocuments` solo cuando los filtros son muy selectivos.
> 3. Mantener el diseño actual aceptando que puede requerir optimización al crecer el volumen.
>
> Para el alcance inicial puede aceptarse la opción 3, pero se recomienda registrarlo como Duda/Pendiente (agregar a la sección §Dudas y Pendientes).
...
> 🟡 **[OBS-R-001] Typo en endpoints de health checks:** La celda anterior muestra `/health/live//health/ready` (doble barra). Debe ser `/health/live` y `/health/ready` como endpoints separados, tal como se describe en RT-B10 y en la sección 6 de Diseño de componentes.
...
> 🔴 **[OBS-R-008] Duda #3 — La política de retención debe resolverse ANTES de implementar los índices:**
> Si la retención es con expiración, MongoDB permite un **TTL index** sobre `RegisteredAt` (o `Timestamp`), que es un tipo de índice especial — no puede combinarse con otros índices sobre el mismo campo. Esto afecta directamente el diseño de índices de la sección 2 ("Modelo de datos persistido").
>
> Si se arranca la implementación con retención indefinida y luego se decide agregar expiración, agregar el TTL index después no requiere migración de datos, pero sí puede causar eliminaciones retroactivas de documentos antiguos al activarse. **Recomendación: cerrar esta duda antes de ejecutar el script de creación de índices en cualquier ambiente.**

# 

---

> ## Resumen de observaciones de revisión — Juan David García Cruz (2026-07-30)
>
> | ID | Sección | Severidad | Descripción breve |
> | :---- | :---- | :---- | :---- |
> | OBS-R-001 | Impacto en infraestructura | 🟡 Menor | Typo `/health/live//health/ready` — doble barra |
> | OBS-R-002 | Componentes involucrados (diagrama) | 🟡 Menor | Routing key `{app}` vs `{aplicativo}` — homogeneizar |
> | OBS-R-003 | RT-B05 | 🔴 Mayor | TTL escalonado (2s/10s/30s) requiere 3 colas Retry separadas, no una sola con `x-message-ttl` |
> | OBS-R-004 | Modelo de datos — Índices | 🔴 Mayor | Índice compuesto debe incluir `Application` para evitar colisión de nombres de entidad entre aplicativos |
> | OBS-R-005 | Contrato API de consulta | 🟡 Menor | Agregar filtro `Operation` a `BitacoraQueryRequest` |
> | OBS-R-006 | Contrato API de consulta | 🔴 Mayor | `TotalCount` con `countDocuments()` puede ser costoso en colecciones grandes; evaluar paginación por cursor |
> | OBS-R-007 | RT-B14 | 🟡 Menor | Clarificar si `Superusuario` también debe tener acceso al endpoint |
> | OBS-R-008 | Dudas y Pendientes #3 | 🔴 Mayor | Política de retención debe cerrarse antes de crear los índices en cualquier ambiente |
